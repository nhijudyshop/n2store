    ...
    if (!checked && item && item.is_verified) {
        ...
        if (window.notificationManager) {
            window.notificationManager.show(
                'Không thể bỏ "Ẩn/Hiện" khi đã "Kiểm Tra". Hãy bỏ check "Đã Kiểm Tra" trước.',
                'warning'
            );
        }
        return;
...
async function markAllChecked() {
    ...
    try {
        const response = await fetch(
            `${TS_API_BASE_URL}/api/sepay/transfer-stats/mark-all-checked`,
            {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    try {
        ...
        if (result.success) {
            ...
            const btn = document.querySelector(
                `button[onclick="transferToStats(${transactionId})"]`
            );
            if (btn) {
    ...
    tsTPOSLookupTimeout = setTimeout(async () => {
        try {
            ...
            if (result.success && result.count > 0) {
                if (result.count === 1) {
                    ...
                    if (single) {
                        ...
                        document.getElementById('tsTPOSLookupName').textContent =
            ...
            if (empty) {
                ...
                empty.innerHTML =
                    '<i data-lucide="alert-circle" style="width: 14px; height: 14px; vertical-align: middle;"></i> Lỗi khi tìm TPOS';
            }
            lucide.createIcons();
        }
    }, 500);
