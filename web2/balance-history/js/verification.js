...
function renderVerificationQueue(tableBody) {
    ...
    tableBody.innerHTML = verificationQueueData
        .map((tx) => {
...
function updateVerificationPagination() {
    ...
    if (pageInfo)
        pageInfo.textContent = `Trang ${verificationCurrentPage} / ${verificationTotalPages}`;
...
async function approveTransaction(transactionId) {
    ...
    if (tx && tx.wallet_processed === true) {
        ...
        console.log(
            `[SECURITY] Blocked approveTransaction for tx ${transactionId} - wallet_processed = true`
        );
        return;
    }
...
async function selectMatchAndApprove(transactionId, selectEl) {
    ...
    try {
        ...
        const linkResponse = await fetch(
            `${API_BASE_URL}/api/v2/balance-history/${transactionId}/link`,
            {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
...
function showChangeModal(transactionId, currentPhone, currentName) {
    ...
    if (tx && tx.wallet_processed === true) {
        showNotification(
            'Không thể thay đổi SĐT - Giao dịch đã được cộng vào ví khách hàng',
            'error'
        );
        console.log(
            `[SECURITY] Blocked showChangeModal for tx ${transactionId} - wallet_processed = true`
        );
        return;
    }
...
async function changeAndApproveTransaction(transactionId, newPhone, newName) {
    ...
    if (tx && tx.wallet_processed === true) {
        ...
        console.log(
            `[SECURITY] Blocked changeAndApproveTransaction for tx ${transactionId} - wallet_processed = true`
        );
        return;
    }
    ...
    try {
        ...
        console.log(
            `[VERIFICATION] Transaction ${transactionId} updated and auto-approved by backend`
        );
        showNotification(
            `Đã thay đổi SĐT thành ${newPhone} và duyệt giao dịch #${transactionId}`,
            'success'
        );
