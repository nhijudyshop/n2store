...
function renderTransactionRow(row) {
    ...
    let rowClass =
        hasPendingMatch || (isSkipped && pendingMatchOptions.length > 0) ? 'row-pending-match' : '';
    ...
    if (hasPendingMatch && pendingMatchOptions.length > 0) {
        ...
        const optionsHtml = pendingMatchOptions
            .map((opt) => {
                ...
                return customers
                    .map((c) => {
                        ...
                        const customerId =
                            c.id || c.customer_id || (c.phone ? `LOCAL_${c.phone}` : '');
        ...
        const hasAliases =
            aliases.length > 1 || (row.linked_customer_phone && customerDisplay.hasInfo);
    ...
    if (hasPendingMatch) {
        ...
        const canManualEntry =
            authManager?.hasDetailedPermission('balance-history', 'manualTransactionEntry') ||
            authManager?.hasDetailedPermission('balance-history', 'edit');
        if (canManualEntry) {
            phoneCell = `
                <div style="display: flex; align-items: center; gap: 4px;">
...
async function toggleHideTransaction(transactionId, hidden) {
    try {
        const response = await fetch(
            `${API_BASE_URL}/api/sepay/transaction/${transactionId}/hidden`,
            {
                method: 'PUT',
                headers: {
        ...
        if (result.success) {
            ...
            if (row) {
                ...
                const shouldRemoveRow =
                    (hidden && currentViewMode === 'visible') ||
                ...
                if (shouldRemoveRow) {
                    ...
                    if (hideBtn) {
                        ...
                        hideBtn.setAttribute(
                            'onclick',
                            `toggleHideTransaction(${transactionId}, ${!hidden})`
                        );
        ...
        if (window.NotificationManager) {
            window.NotificationManager.showNotification(
                'Khong the cap nhat trang thai an',
                'error'
            );
        } else {
...
async function ignoreGap(referenceCode) {
    try {
        ...
        if (window.NotificationManager) {
            window.NotificationManager.showNotification(
                'Loi khi bo qua gap: ' + error.message,
                'error'
            );
        }
    }
...
async function rescanGaps() {
    ...
    try {
        ...
        if (window.NotificationManager) {
            window.NotificationManager.showNotification(
                'Gap detection da bi tat de toi uu hieu suat',
                'info'
            );
        }
...
filterGapsOnlyBtn?.addEventListener('click', () => {
    ...
    if (showGapsOnly) {
        ...
        if (window.NotificationManager) {
            window.NotificationManager.showNotification(
                'Dang hien thi chi giao dich thieu',
                'info'
            );
        }
    } else {
...
async function fetchMissingTransaction(referenceCode) {
    if (window.NotificationManager) {
        window.NotificationManager.showNotification(
            `Dang lay giao dich ${referenceCode}...`,
            'info'
        );
    }
    ...
    try {
        ...
        if (result.success) {
            if (window.NotificationManager) {
                window.NotificationManager.showNotification(
                    `Da lay duoc giao dich ${referenceCode}!`,
                    'success'
                );
            }
        ...
        if (window.NotificationManager) {
            window.NotificationManager.showNotification(
                `Khong the lay GD ${referenceCode}: ${error.message}`,
                'error'
            );
        }
    }
