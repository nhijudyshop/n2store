...
const CustomerInfoManager = {
    ...
    async syncFromDatabase() {
        try {
            ...
            if (result.success && result.data) {
                ...
                console.log(
                    '[CUSTOMER-INFO] ✅ Synced from database:',
    ...
    async saveCustomerInfo(uniqueCode, customerInfo) {
        ...
        const isNewPhone =
            customerInfo.phone && (!existingInfo || existingInfo.phone !== customerInfo.phone);
        ...
        try {
            ...
            if (result.success) {
                console.log(
                    '[CUSTOMER-INFO] ✅ Saved to database:',
                    uniqueCode,
                    'customerId:',
                    result.customerId
                );
    ...
    async syncToFirebase(customerInfo) {
        ...
        try {
            ...
            if (!querySnapshot.empty) {
                ...
                console.warn(
                    '[CUSTOMER-INFO] ⚠️ Customer not found in Firebase (phone: ' +
                        customerInfo.phone +
                        '). Skipping sync. Customer must be created in Customer Hub first.'
                );
            }
        } catch (error) {
    ...
    searchCustomers(query) {
        ...
        for (const [uniqueCode, info] of Object.entries(allData)) {
            if (info.name?.toLowerCase().includes(lowerQuery) || info.phone?.includes(query)) {
                results.push({
                    uniqueCode,
                    name: info.name,
                    phone: info.phone,
