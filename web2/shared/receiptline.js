// #Note: VENDORED ReceiptLine (receiptline/receiptline, Apache-2.0) — render bill SVG vector. KHÔNG sửa tay; cập nhật từ npm. | web2-bill-service.js dùng window.receiptline.transform.
/*
...
(function () {
    ...
    function transform(doc, printer) {
        ...
        const res = doc
            .normalize()
            .split(/\n|\r\n|\r/)
            .map((line) => createLine(parseLine(line, state), ptr, state));
    ...
    function parseOption(printer) {
        ...
        p.cpl = p.cpl || 48;
        p.encoding =
            /^(cp(437|85[28]|86[0356]|1252|93[26]|949|950)|multilingual|shiftjis|gb18030|ksc5601|big5|tis620)$/.test(
                p.encoding
            )
                ? p.encoding
                : 'cp437';
        ...
        p.command = Object.assign(
            {},
            (typeof p.command !== 'object' ? commands[p.command] : p.command) || commands.svg
        );
        return p;
    }
                ...
                if (/^\{[^{}]*\}$/.test(element)) {
                        ...
                        .reduce((obj, member) => {
                            ...
                            if (
                                ...
                                member.replace(
                                    /^[\t ]*([A-Za-z_]\w*)[\t ]*:[\t ]*([^\t ].*?)[\t ]*$/,
                                    (match, key, value) =>
                                        (obj[key.replace(/^[abciotwx_]$/, (m) => abbr[m])] =
                    ...
                    if (array.length === 1) {
                        ...
                        if ('border' in result.property) {
                            ...
                            state.border = /^(line|space|none)$/.test(c)
                                ? border[c.toLowerCase()]
                                : /^\d+$/.test(c) && Number(c) <= 2
                                  ? Number(c)
                                  : 1;
                            // start rules
                        ...
                        if ('width' in result.property) {
                            ...
                            state.width = width.find((c) => /^auto$/.test(c))
                                ? []
                                : width.map((c) =>
                                      /^\*$/.test(c) ? -1 : /^\d+$/.test(c) ? Number(c) : 0
                                  );
                        }
                        ...
                        if ('align' in result.property) {
                            ...
                            state.align = /^(left|center|right)$/.test(c)
                                ? align[c.toLowerCase()]
                                : 1;
                        }
                        ...
                        if ('option' in result.property) {
                            ...
                            state.option = {
                                type:
                                    option.find((c) =>
                                        /^(upc|ean|jan|code39|itf|codabar|nw7|code93|code128|qrcode)$/.test(
                                            c
                                        )
                                    ) || 'code128',
                                width: Number(
                                    option.find(
                                        (c) => /^\d+$/.test(c) && Number(c) >= 2 && Number(c) <= 4
                                    ) || '2'
                                ),
                                height: Number(
                                    option.find(
                                        (c) =>
                                            /^\d+$/.test(c) && Number(c) >= 24 && Number(c) <= 240
                                    ) || '72'
                                ),
                                hri: !!option.find((c) => /^hri$/.test(c)),
                                cell: Number(
                                    option.find(
                                        (c) => /^\d+$/.test(c) && Number(c) >= 3 && Number(c) <= 8
                                    ) || '3'
                                ),
                ...
                if (state.width.length === 0) {
                    ...
                    result.width =
                        w.length > 0
                            ? w.reduce(
                                  (a, c) => a + c,
                                  result.border < 0 ? w.length + 1 : (w.length - 1) * result.border
                              )
                            : 0;
                }
                // set line alignment
    ...
    function parseEscape(chars) {
        return (
            chars
                ...
                .replace(/\\x([\dA-Fa-f]{2})/g, (match, hex) =>
        ...
        if (text) {
            columns = columns.slice(
                ...
                Math.floor(
                    column.border < 0
                        ? (printer.cpl - 1) / 2
                        : (printer.cpl + column.border) / (column.border + 1)
                )
            );
        }
        // fixed columns
            ...
            for (let j = 0; j < row; j++) {
                ...
                if (state.line === 'running') {
                    ...
                    const height = cols.reduce(
                        (a, col) => (j < col.length ? Math.max(a, col[j].height) : a),
                        1
                    );
                ...
                cols.forEach((col, i) => {
                    ...
                    // if wrapped text is empty
                        ...
                        res +=
                            printer.command.normal() + printer.command.text(' ', printer.encoding);
                    }
        ...
        column.text.forEach((text, i) => {
            ...
            if (i % 2 === 0) {
                ...
                while (t.length > 0) {
                    ...
                    while (j < t.length) {
                        w =
                            printer.command.measureText(t[j], printer.encoding) *
                            (wh < 2 ? wh + 1 : wh - 1);
    ...
    const barcode = (() => {
        ...
        const code128a = (x, s, d) => {
            ...
            s = s.replace(
                /^((?!\d{4,})[\x00-_])+/,
                (m) => (m.split('').forEach((c) => d.push((c.charCodeAt(0) + 64) % 96)), '')
            );
            s = s.replace(
                /^\d(?=(\d\d){2,}(\D|$))/,
                (m) => (d.push((m.charCodeAt(0) + 64) % 96), '')
            );
            const t = s.slice(1);
            const p = t.search(/[^ -_]/);
            if (/^\d{4,}/.test(s)) {
        ...
        const code128b = (x, s, d) => {
            ...
            s = s.replace(
                /^((?!\d{4,})[ -\x7f])+/,
                (m) => (m.split('').forEach((c) => d.push(c.charCodeAt(0) - 32)), '')
            );
        ...
        const c93 = {
            ...
            code: '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ-. $/+%dcsp'
                .split('')
                .reduce((a, c, i) => ((a[c] = i), a), {}),
            element:
        ...
        const code93 = (symbol) => {
            ...
            if (s.length > 0) {
                ...
                const d = s
                    .split('')
                    .reduce((a, c) => a + c93.escape[c.charCodeAt(0)], '')
                    .split('')
                    .map((c) => c93.code[c]);
            ...
            if (s.length > 0) {
                ...
                const m =
                    s
                        .toUpperCase()
                        .split('')
                        .reduce((a, c) => a + nw7[c] + '2', q)
                        .slice(0, -1) + q;
                r.widths = m.split('').map((c) => (parseInt(c, 16) * symbol.width + 1) >> 1);
                const w = [25, 39, 50, 3, 5, 6];
                r.length =
                    s.length * w[symbol.width - 2] -
                    (s.match(/[\d\-$]/g) || []).length * w[symbol.width + 1] +
                    symbol.width * (symbol.quietZone ? 19 : -1);
                r.height = symbol.height;
            }
            return r;
        };
        ...
        const itf = (symbol) => {
            ...
            if (s.length > 0) {
                ...
                r.length =
                    s.length * w[symbol.width - 2] +
                    w[symbol.width + 1] +
                    symbol.width * (symbol.quietZone ? 20 : 0);
                r.height = symbol.height;
            }
            return r;
        };
        ...
        const code39 = (symbol) => {
            ...
            if (s.length > 0) {
                ...
                const m =
                    s
                        .split('')
                        .reduce((a, c) => a + c39[c] + '2', q)
                        .slice(0, -1) + q;
                r.widths = m.split('').map((c) => (parseInt(c, 16) * symbol.width + 1) >> 1);
                const w = [29, 45, 58];
                r.length =
                    s.length * w[symbol.width - 2] + symbol.width * (symbol.quietZone ? 19 : -1);
                r.height = symbol.height;
            }
            return r;
        };
        ...
        const upce = (symbol) => {
            ...
            if (d.length > 0) {
                ...
                d[7] = 0;
                d[7] =
                    (10 - (upcetoa(d).reduce((a, c, i) => a + c * (3 - (i % 2) * 2), 0) % 10)) % 10;
                // generate HRI
                r.hri = symbol.hri;
                r.text = d.join('');
                // generate bars and spaces
        ...
        const evaluateMask = (m) => {
            ...
            for (let r = 0; r < size; r++) {
                ...
                for (let i = 1; i < h.length - 5; i += 2) {
                    if (
                        p.every((c, j) => c === h[i + j]) &&
                        (i === 1 || h[i - 1] >= 4 || i === h.length - 6 || h[i + 5] >= 4)
                    ) {
                        score += 40;
                    }
                }
                for (let i = 1; i < v.length - 5; i += 2) {
                    if (
                        p.every((c, j) => c === v[i + j]) &&
                        (i === 1 || v[i - 1] >= 4 || i === v.length - 6 || v[i + 5] >= 4)
                    ) {
                        score += 40;
                    }
                }
    ...
    const _svg = {
        ...
        close: function () {
            ...
            return (
                `<svg width="${this.svgWidth}px" height="${this.svgHeight}px" viewBox="0 0 ${this.svgWidth} ${this.svgHeight}" preserveAspectRatio="xMinYMin meet" xmlns="http://www.w3.org/2000/svg">${p.style}` +
        ...
        vr: function (widths, height) {
            ...
            const path =
                `<path d="` +
                widths.reduce((a, width) => a + `m${w * width + w},${-v}v${v}`, `M${u},0v${v}`) +
        ...
        vrstart: function (widths) {
            ...
            const path =
                ...
                widths
                    .reduce(
                        (a, width) => a + `h${w * width}h${u}v${w}m0,${-w}h${u}`,
                        `M${u},${w + w}v${-u}q0,${-u},${u},${-u}`
                    )
                    .replace(/h\d+v\d+m0,-\d+h\d+$/, `q${u},0,${u},${u}v${u}`) +
                `" fill="none" stroke="#000" stroke-width="2"/>`;
            this.svgContent += `<g transform="translate(${this.lineMargin * w},${this.svgHeight})">${path}</g>`;
            return '';
        },
        ...
        vrhr: function (widths1, widths2, dl, dr) {
            ...
            const path1 =
                ...
                widths1
                    .reduce(
                        (a, width) => a + `h${w * width}h${u}v${-w}m0,${w}h${u}`,
                        `M${u},0` + (dl > 0 ? `v${u}q0,${u},${u},${u}` : `v${w}h${u}`)
                    )
                    .replace(
                        /h\d+v-\d+m0,\d+h\d+$/,
                        dr < 0 ? `q${u},0,${u},${-u}v${-u}` : `h${u}v${-w}`
                    ) +
                `" fill="none" stroke="#000" stroke-width="2"/>`;
            ...
            const path2 =
                ...
                widths2
                    .reduce(
                        (a, width) => a + `h${w * width}h${u}v${w}m0,${-w}h${u}`,
                        `M${u},${w + w}` + (dl < 0 ? `v${-u}q0,${-u},${u},${-u}` : `v${-w}h${u}`)
                    )
                    .replace(
                        /h\d+v\d+m0,-\d+h\d+$/,
                        dr > 0 ? `q${u},0,${u},${u}v${u}` : `h${u}v${w}`
        ...
        text: function (text, encoding) {
            ...
            const tspan = this.arrayFrom(text, encoding).reduce((a, c) => {
                ...
                return (
                    a +
                    `<tspan x="${r}">${c.replace(/[ &<>]/g, (r) => ({ ' ': '&#xa0;', '&': '&amp;', '<': '&lt;', '>': '&gt;' })[r])}</tspan>`
                );
            }, '');
            const attr = Object.keys(this.textAttributes).reduce(
                (a, key) => a + ` ${key}="${this.textAttributes[key]}"`,
                ''
            );
        ...
        image: function (image) {
            ...
            let imgHeight = 0;
            png.replace(
                /^\x89PNG\x0d\x0a\x1a\x0a\x00\x00\x00\x0dIHDR(.{4})(.{4})/,
                (match, w, h) => {
                    imgWidth =
                        (w.charCodeAt(0) << 24) |
                        (w.charCodeAt(1) << 16) |
            ...
            const margin = Math.floor(
                this.lineMargin * this.charWidth +
                    ((this.lineWidth * this.charWidth - imgWidth) * this.lineAlign) / 2
            );
        ...
        qrcode: function (symbol, encoding) {
            if (symbol.data.length > 0) {
                ...
                const margin = Math.floor(
                    this.lineMargin * this.charWidth +
                        ((this.lineWidth * this.charWidth - w * c) * this.lineAlign) / 2
                );
        ...
        barcode: function (symbol, encoding) {
            ...
            if ('length' in bar) {
                ...
                if (bar.hri) {
                    const m = Math.floor(
                        (width - (this.measureText(bar.text, encoding) - 1) * this.charWidth) / 2
                    );
                    const tspan = this.arrayFrom(bar.text, encoding).reduce(
                        (a, c, i) =>
                            a +
                            `<tspan x="${m + this.charWidth * i}">${c.replace(/[ &<>]/g, (r) => ({ ' ': '&#xa0;', '&': '&amp;', '<': '&lt;', '>': '&gt;' })[r])}</tspan>`,
                        ''
                    );
                ...
                const margin = Math.floor(
                    this.lineMargin * this.charWidth +
                        ((this.lineWidth * this.charWidth - width) * this.lineAlign) / 2
                );
                this.svgContent += `<g transform="translate(${margin},${this.svgHeight})">${path}</g>`;
    ...
    const _text = {
        ...
        vrhr: function (widths1, widths2, dl, dr) {
            const r1 =
                ' '.repeat(Math.max(-dl, 0)) +
                widths1.reduce((a, w) => a + '-'.repeat(w) + '+', '+') +
                ' '.repeat(Math.max(dr, 0));
            const r2 =
                ' '.repeat(Math.max(dl, 0)) +
                widths2.reduce((a, w) => a + '-'.repeat(w) + '+', '+') +
                ' '.repeat(Math.max(-dr, 0));
            return (
                ' '.repeat(this.left) +
                r2.split('').reduce((a, c, i) => a + this.vrtable[c][r1[i]], '')
            );
        },
        ...
        // print text:
        text: function (text, encoding) {
            const d = this.arrayFrom(text, encoding).reduce(
                (a, c) => a + c + ' '.repeat(this.measureText(c, encoding) * (this.scale - 1)),
                ''
            );
    ...
    const _thermal = {
        ...
        vrstart: (widths) =>
            '\x1cC0\x1c.\x1bt\x01' +
            widths.reduce((a, w) => a + '\x95'.repeat(w) + '\x91', '\x9c').slice(0, -1) +
            '\x9d',
        // stop rules: FS C n FS . ESC t n ...
        vrstop: (widths) =>
            '\x1cC0\x1c.\x1bt\x01' +
            widths.reduce((a, w) => a + '\x95'.repeat(w) + '\x90', '\x9e').slice(0, -1) +
            '\x9f',
        ...
        vrhr: function (widths1, widths2, dl, dr) {
            const r1 =
                ' '.repeat(Math.max(-dl, 0)) +
                widths1
                    .reduce((a, w) => a + '\x95'.repeat(w) + '\x90', dl > 0 ? '\x9e' : '\x9a')
                    .slice(0, -1) +
                (dr < 0 ? '\x9f' : '\x9b') +
                ' '.repeat(Math.max(dr, 0));
            const r2 =
                ' '.repeat(Math.max(dl, 0)) +
                widths2
                    .reduce((a, w) => a + '\x95'.repeat(w) + '\x91', dl < 0 ? '\x9c' : '\x98')
                    .slice(0, -1) +
                (dr > 0 ? '\x9d' : '\x99') +
                ' '.repeat(Math.max(-dr, 0));
            return (
        ...
        wh: (wh) =>
            '\x1d!' +
            (wh < 3 ? $(((wh & 1) << 4) | ((wh >> 1) & 1)) : $(((wh - 2) << 4) | (wh - 2))),
            ...
            switch (encoding) {
                ...
                case 'tis620':
                    return (
                        ...
                        this.arrayFrom(text, encoding).reduce(
                            (a, c) => a + '\x00' + iconv.encode(c, encoding).toString('binary'),
                            ''
                        )
                    );
                default:
                    return (
                        this.codepage[encoding] + iconv.encode(text, encoding).toString('binary')
                    );
            }
        },
        // feed new line: LF
        ...
        image: function (image) {
            ...
            let r = this.upsideDown
                ? this.area(right + this.marginRight - this.margin, width, left) +
                  this.align(2 - align)
                : '';
        ...
        qrcode: function (symbol, encoding) {
            let r = this.upsideDown
                ? this.area(this.right + this.marginRight - this.margin, this.width, this.left) +
                  this.align(2 - this.alignment)
                : '';
            if (symbol.data.length > 0) {
                const matrix = qrcode.generate(symbol);
                if (this.upsideDown) {
        ...
        barcode: function (symbol, encoding) {
            let d = iconv
                .encode(symbol.data, encoding === 'multilingual' ? 'ascii' : encoding)
                .toString('binary');
            const b =
                this.bartype[symbol.type] +
                Number(/upc|[ej]an/.test(symbol.type) && symbol.data.length < 9);
            switch (b) {
    ...
    const _sii = {
        ...
        open: function (printer) {
            ...
            this.marginRight = printer.marginRight;
            return (
                '\x1b@\x1da\x00\x1bM0\x1b \x00\x1cS\x00\x00' +
                (this.spacing ? '\x1b2' : '\x1b3\x00') +
                '\x1b{' +
                $(this.upsideDown) +
                '\x1c.'
        ...
        area: function (left, width, right) {
            ...
            this.right = right;
            const m =
                (this.upsideDown ? this.marginRight + right : this.margin + left) * this.charWidth;
            const w = width * this.charWidth;
            return '\x1dL' + $(m & 255, (m >> 8) & 255) + '\x1dW' + $(w & 255, (w >> 8) & 255);
        },
        // image split size
        ...
        qrcode: function (symbol, encoding) {
            let r = this.upsideDown
                ? this.area(this.right, this.width, this.left) + this.align(2 - this.alignment)
                : '';
            if (symbol.data.length > 0) {
                const matrix = qrcode.generate(symbol);
                if (this.upsideDown) {
        ...
        code93: function (data) {
            ...
            if (s.length > 0) {
                const d = s
                    .split('')
                    .reduce((a, c) => a + this.c93.escape[c.charCodeAt(0)], '')
                    .split('')
    ...
    const _fit = {
        ...
        image: function (image) {
            ...
            return (
                (this.upsideDown && align === 2 ? this.area(right, width, left) : '') + s.join('')
            );
        },
        // print QR Code: GS ( k pL pH cn fn n1 n2 GS ( k pL pH cn fn n GS ( k pL pH cn fn n GS ( k pL pH cn fn m d1 ... dk GS ( k pL pH cn fn m
        qrcode: function (symbol, encoding) {
            let r =
                this.upsideDown && this.alignment === 2
                    ? this.area(this.right, this.width, this.left)
                    : '';
            if (symbol.data.length > 0) {
                const matrix = qrcode.generate(symbol);
                const w = matrix.length * symbol.cell;
    ...
    const _impact = {
        ...
        hr: function (width) {
            return (
                '\x1b!' + $(this.font) + ' '.repeat(this.left) + '\x1bt\x01' + '\x95'.repeat(width)
            );
        },
        ...
        text: function (text, encoding) {
            const t = iconv
                .encode(text, encoding === 'multilingual' ? 'ascii' : encoding)
                .toString('binary');
            const d =
                '\x1b!' +
                $(this.style) +
                ...
                r +=
                    this.red
                        ...
                        .reduce(
                            (a, c) => {
                                const s =
                                    a + '\x1b!' + $(this.font) + ' '.repeat(c.index - p) + c.data;
                                p = c.index + c.length;
                                return s;
                            },
                            '\x1br\x01\x1b!' + $(this.font) + ' '.repeat(this.left)
                        ) + '\x0d\x1br\x00';
            }
    ...
    const _star = {
        ...
        open: function (printer) {
            ...
            return (
                '\x1b@\x1b\x1ea\x00' +
                (printer.encoding === 'tis620' ? '\x1b\x1eR\x01' : '') +
                '\x1b\x1eF\x00\x1b 0\x1bs00' +
                (this.spacing ? '\x1bz1' : '\x1b0') +
                (this.upsideDown ? '\x0f' : '\x12')
            );
        },
        ...
        vrlf: function (vr) {
            return (
                (this.upsideDown ? this.lf() : '') +
                (vr === this.upsideDown && this.spacing ? '\x1bz1' : '\x1b0') +
                (this.upsideDown ? '' : this.lf())
            );
        },
        // cut paper: ESC d n
        ...
        barcode: function (symbol, encoding) {
            ...
            const u =
                symbol.type === 'itf'
                    ? [49, 56, 50][symbol.width - 2]
                    : symbol.width + (/^(code39|codabar|nw7)$/.test(symbol.type) ? 50 : 47);
            return d.length > 0
                ? '\x1bb' + $(b, symbol.hri ? 50 : 49, u, symbol.height) + d + '\x1e'
                : '';
        },
        // barcode types:
        bartype: {
        ...
        code128: (data) =>
            data
                .replace(/((?!^[\x00-\x7f]+$).)*/, '')
                .replace(/%/g, '%0')
                .replace(/[\x00-\x1f]/g, (m) => '%' + $(m.charCodeAt(0) + 64))
                .replace(/\x7f/g, '%5'),
    };
    ...
    const _dot = {
        ...
        open: function (printer) {
            ...
            this.margin = printer.margin;
            return (
                '\x1b@\x1b\x1ea\x00\x1b' +
                ['M', 'P', ':'][this.font] +
                '\x1b \x00\x1bs\x00\x00' +
                (this.spacing ? '\x1bz\x01' : '\x1b0') +
                (this.upsideDown ? '\x0f' : '\x12')
            );
        },
        // finish printing: ESC GS ETX s n1 n2 EOT
    ...
    const _sbcs = {
        ...
        vrstart: (widths) =>
            '\x1b\x1dt\x01' +
            widths.reduce((a, w) => a + '\xc4'.repeat(w) + '\xc2', '\xda').slice(0, -1) +
            '\xbf',
        // stop rules: ESC GS t n ...
        vrstop: (widths) =>
            '\x1b\x1dt\x01' +
            widths.reduce((a, w) => a + '\xc4'.repeat(w) + '\xc1', '\xc0').slice(0, -1) +
            '\xd9',
        // print vertical and horizontal rules: ESC GS t n ...
        vrhr: function (widths1, widths2, dl, dr) {
            const r1 =
                ' '.repeat(Math.max(-dl, 0)) +
                widths1.reduce((a, w) => a + '\xc4'.repeat(w) + '\xc1', '\xc0').slice(0, -1) +
                '\xd9' +
                ' '.repeat(Math.max(dr, 0));
            const r2 =
                ' '.repeat(Math.max(dl, 0)) +
                widths2.reduce((a, w) => a + '\xc4'.repeat(w) + '\xc2', '\xda').slice(0, -1) +
                '\xbf' +
                ' '.repeat(Math.max(-dr, 0));
            return (
                '\x1b\x1dt\x01' + r2.split('').reduce((a, c, i) => a + this.vrtable[c][r1[i]], '')
            );
        },
        // ruled line composition
        vrtable: {
    ...
    const _stargraphic = {
        ...
        open: function (printer) {
            ...
            this.right = 0;
            this.margin =
                (printer.upsideDown ? printer.marginRight : printer.margin) * this.charWidth;
        ...
        image: function (image) {
            ...
            const m =
                this.margin +
                Math.max(
                    ((this.upsideDown ? right : left) * this.charWidth +
                        (width * this.charWidth - w) * (this.upsideDown ? 2 - align : align)) >>
                        1,
                    0
                );
            const l = (m + w + 7) >> 3;
    ...
    const _generic = {
        ...
        open: function (printer) {
            ...
            this.marginRight = printer.marginRight;
            return (
                '\x1b@\x1da\x00\x1bM\x00\x1b \x00\x1cS\x00\x00' +
                (this.spacing ? '\x1b2' : '\x1b3\x00') +
                '\x1b{' +
                $(this.upsideDown) +
                '\x1c.'
        ...
        vr: function (widths, height) {
            return widths.reduce(
                (a, w) => a + this.relative(w) + '\x96',
                '\x1d!' + $(height - 1) + '\x1cC\x00\x1c.\x1bt\x01\x96'
            );
        },
        // start rules: FS C n FS . ESC t n ...
        vrstart: (widths) =>
            '\x1cC\x00\x1c.\x1bt\x01' +
            widths.reduce((a, w) => a + '\x95'.repeat(w) + '\x91', '\x9c').slice(0, -1) +
            '\x9d',
        // stop rules: FS C n FS . ESC t n ...
        vrstop: (widths) =>
