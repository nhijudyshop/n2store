...
(async () => {
    ...
    const nav = [];
    page.on('framenavigated', f => { if (f === page.mainFrame()) nav.push({ ts: Date.now(), url: f.url() }); });
    ...
    for (const label of targets) {
        try {
            ...
            const locator = page.locator(`a:has-text("${label}"), [role="menuitem"]:has-text("${label}"), .ui-menuitem-text:has-text("${label}")`).first();
            const count = await locator.count();
            if (count === 0) { results.push({ label, found: false }); continue; }
            ...
            console.log(`[${label}] -> ${urlAfter} (title=${title.slice(0, 60)}) audio?=${hasAudio}`);

            // Dump HTML snapshot để inspect
            const snap = path.resolve(__dirname, '..', 'docs', `oncallcx-page-${label.replace(/\s+/g, '_')}.html`);
            fs.writeFileSync(snap, await page.content());

            // Quay lại dashboard trước khi thử label kế
            await page.goto('https://pbx-ucaas.oncallcx.vn/portal/pbxDashboard.xhtml', { waitUntil: 'networkidle' });
    ...
    fs.writeFileSync(path.resolve(__dirname, '..', 'docs', 'oncallcx-menu-click-results.json'), JSON.stringify({ navigations: nav, results }, null, 2));
    console.log('\n=== NAV HISTORY ===');
