...
async function main() {
    ...
    const pageInfo = await page.evaluate(() => {
        const pg = document.querySelector(
            '#content\\:calls\\:calls_paginator_top, #content\\:calls\\:calls_paginator_bottom'
        );
    ...
    const nextPageBtn = page
        .locator(
            '#content\\:calls\\:calls_paginator_top a.ui-paginator-next, #content\\:calls\\:calls_paginator_bottom a.ui-paginator-next'
        )
        .first();
    ...
    while (pagesVisited < MAX_PAGES) {
        ...
        for (const r of eligible) {
            ...
            const rowLocator = page
                .locator(`#content\\:calls\\:calls tbody tr[data-ri="${r.dataRi}"]`)
                .first();
            try {
                ...
                await page
                    .waitForResponse(
                        (r) =>
                            r.url().includes('pbxCalls.xhtml') && r.request().method() === 'POST',
                        { timeout: 5000 }
                    )
                    .catch(() => {});
            ...
            if (disabled !== null) {
                ...
                console.log(
                    `  [skip] ${r.start} ${r.from}->${r.outPub} dur=${r.duration} (btn disabled)`
                );
                continue;
            }
            ...
            if (DRY_RUN) {
                ...
                console.log(
                    `  [dry]  ${r.start} ${r.from}->${r.outPub} dur=${r.duration} RECORDING AVAIL`
                );
                continue;
            }
        ...
        if (MAX && downloaded >= MAX) {
            console.log(`[info] Reached MAX=${MAX}, stopping.`);
            break;
        }

        // Next page
        const hasNext =
            (await nextPageBtn.count()) > 0 &&
            !(await nextPageBtn.getAttribute('class'))?.includes('ui-state-disabled');
        if (!hasNext) {
        ...
        await Promise.all([
            page
                .waitForResponse((r) => r.url().includes('pbxCalls.xhtml'), { timeout: 10000 })
                .catch(() => {}),
            nextPageBtn.click(),
        ]);
        ...
        JSON.stringify(
            {
                ...
                pagesVisited,
                downloaded,
                eligibleTotal: summary.filter(
                    (s) => s.status === 'dry-has-recording' || s.status === 'downloaded'
                ).length,
                summary,
                outDir: OUT_DIR,
            },
