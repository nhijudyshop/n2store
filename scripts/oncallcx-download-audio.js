...
async function main() {
    ...
    const submitBtn = page
        .locator(
            'button[type="submit"], button:has-text("Login"), button:has-text("Đăng nhập"), input[type="submit"]'
        )
        .first();
    await Promise.all([
        ...
        submitBtn
            .count()
            .then((c) =>
                c > 0 ? submitBtn.click() : page.locator('input[type="password"]').press('Enter')
            ),
    ]);
    ...
    console.log(
        `[info] Login ${hasAuth ? 'OK' : 'UNKNOWN'} (ANAUTH=${hasAuth}). URL: ${page.url()}`
    );
    if (!hasAuth) {
    ...
    const staticLinks = await page.evaluate(() => {
        ...
        document.querySelectorAll('a[href]').forEach((a) => {
            const href = a.href;
            if (
                /\.(wav|mp3|ogg|m4a)(\?|$)/i.test(href) ||
                /downloadRecord|playRecord|record|audio/i.test(href)
            ) {
                out.push({
                    kind: 'anchor',
                    src: href,
        ...
        document
            .querySelectorAll('button, [onclick], [data-url], [data-src], [data-record]')
            .forEach((el) => {
                const attrs = ['onclick', 'data-url', 'data-src', 'data-record', 'data-file'];
                attrs.forEach((a) => {
                    const v = el.getAttribute(a);
                    if (v && /\.(wav|mp3)|record|audio/i.test(v))
                        out.push({
                            kind: 'button-attr',
                            attr: a,
                            value: v,
    ...
    const playButtons = page.locator(
        'button:has-text("Play"), button[title*="play" i], [class*="play"], i[class*="play"]'
    );
    ...
    staticLinks.forEach((s) => {
        if (s.src && /^https?:/i.test(s.src)) allUrls.add(s.src);
    });
