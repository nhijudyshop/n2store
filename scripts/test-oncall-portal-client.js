...
(async () => {
    ...
    dash.phones.slice(0, 5).forEach(p => console.log(` - ${p.internalNumber} | ${p.phoneName} | ${p.status}`));

    console.log('\n=== EXTENSIONS ===');
    const exts = await client.listExtensions();
    console.log(`Total: ${exts.extensions.length}`);
    exts.extensions.slice(0, 5).forEach(e => console.log(` - ext=${e.extension} | name=${e.displayedName} | user=${e.pbxUser}`));

    console.log('\n=== CALLS (page 1) ===');
    const calls = await client.listCalls({ page: 1 });
    console.log(`Total rows: ${calls.calls.length}`);
    calls.calls.slice(0, 5).forEach(c => console.log(` - ${c.start} | ${c.from}->${c.to} | dur=${c.duration} | rec=${c.hasRecording} | rk=${c.rowKey}`));
    ...
    dids.publicNumbers.slice(0, 3).forEach(d => console.log(` - ${d.publicNumber} | ${d.name} | -> ${d.destination}`));
