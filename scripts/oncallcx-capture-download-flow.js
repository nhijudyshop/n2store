...
(async () => {
    ...
    page.on('response', async r => {
        if (r.url().includes('oncallcx') && r.request().method() !== 'GET') {
            let body = null;
            try { const b = await r.body(); body = b.slice(0, 2000).toString('utf8'); } catch {}
            reqs.push({ phase: 'res', status: r.status(), url: r.url(), headers: r.headers(), body, ts: Date.now() });
        } else if (r.url().includes('oncallcx')) {
            reqs.push({ phase: 'res', status: r.status(), url: r.url(), headers: { 'content-type': r.headers()['content-type'], 'content-length': r.headers()['content-length'] }, ts: Date.now() });
        }
    });
    ...
    await Promise.all([page.waitForLoadState('networkidle').catch(() => {}), page.locator('button[type="submit"]').first().click()]);
    ...
    console.log('Recent POSTs:', reqs.filter(r => r.phase === 'req' && r.method === 'POST').length);
    const preSelectReqs = reqs.length;

    // Select row 0 (first with recording)
    const radio = page.locator('#content\\:calls\\:calls tbody tr[data-ri="0"] .ui-radiobutton-box').first();
    await radio.scrollIntoViewIfNeeded();
    await radio.click();
    await page.waitForTimeout(1500);

    console.log('\n=== SELECT ROW 0 — AJAX POST ===');
    const selectReqs = reqs.slice(preSelectReqs).filter(r => r.phase === 'req' && r.method === 'POST');
    selectReqs.forEach(r => {
        console.log('URL:', r.url);
        console.log('Headers:');
        Object.entries(r.headers).forEach(([k, v]) => console.log(`  ${k}: ${String(v).slice(0, 120)}`));
    ...
    const rowKey = await page.evaluate(() => {
        return document.querySelector('#content\\:calls\\:calls tbody tr[data-ri="0"]')?.getAttribute('data-rk');
    });
    ...
    dlReqs.forEach(r => {
        if (r.phase === 'req') {
            ...
            console.log(`<<< ${r.status} ${r.url} (ct=${r.headers?.['content-type'] || '?'}, len=${r.headers?.['content-length'] || '?'})`);
            if (r.body) console.log('Response body:', r.body.slice(0, 400));
        }
    });

    // Save full trace
    fs.writeFileSync(path.resolve(__dirname, '..', 'docs', 'oncallcx', 'download-flow-trace.json'),
        JSON.stringify({ rowKey, selectRequests: selectReqs, downloadRequests: dlReqs, filename: fname, size: sz }, null, 2));
