...
async function main() {
    ...
    const pages = [...new Set(links.map(l => {
        try { return new URL(l.href).pathname; } catch { return null; }
    }).filter(Boolean))].filter(p => p.endsWith('.xhtml'));

    // Keywords liên quan call / recording
    const candidates = links.filter(l => /call|record|audio|voicemail|cdr|history/i.test(l.href + ' ' + l.text));

    const out = { loginAs: creds.username, totalLinks: links.length, pages, candidates, allLinks: links };
