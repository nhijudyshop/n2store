...
(async () => {
    ...
    for (const url of CANDIDATES) {
        ...
        try {
            ...
            results.push({ url, finalUrl, title: title.slice(0, 80), is404, status: resp?.status() });
            console.log(`${is404 ? '❌' : '✅'} ${url.padEnd(34)} -> ${finalUrl.replace('https://pbx-ucaas.oncallcx.vn/portal/', '')} | ${title.slice(0, 60)}`);
        } catch (e) {
            results.push({ url, error: e.message });
            console.log(`⚠️  ${url.padEnd(34)} ERROR ${e.message.slice(0, 60)}`);
        }
    }
    fs.writeFileSync(path.resolve(__dirname, '..', 'docs', 'oncallcx-url-probe.json'), JSON.stringify(results, null, 2));
    console.log('\n=== VALID PAGES ===');
    results.filter(r => !r.is404 && !r.error).forEach(r => console.log(`  ${r.url} -> ${r.title}`));
    await browser.close();
