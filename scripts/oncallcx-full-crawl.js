...
async function inspectPage(page, info) {
    ...
    const parsed = await page.evaluate(() => {
        const tables = [...document.querySelectorAll('.ui-datatable, table[id]')].map((t) => ({
            ...
            headers: [...t.querySelectorAll('thead th .ui-column-title, thead th')]
        ...
        const buttons = [...document.querySelectorAll('button[id], a.ui-button[id]')]
            .map((b) => ({
                id: b.id,
            ...
            .filter((b) => b.text);
        const viewState =
            document.querySelector('input[name="javax.faces.ViewState"]')?.value || null;
        const forms = [...document.querySelectorAll('form[id]')].map((f) => ({
            id: f.id,
            action: f.action,
            method: f.method,
        }));
        const pagLabel =
            document.querySelector('.ui-paginator-current')?.textContent?.trim() || null;
        const errorBox =
            document.querySelector('.ui-messages-error, .ui-message-error')?.textContent?.trim() ||
            null;
...
(async () => {
    ...
    for (const p of PAGES) {
        ...
        console.log(
            `  title: ${r.title}  404: ${r.is404}  tables: ${r.tables?.length || 0}  buttons: ${r.buttons?.length || 0}`
        );
        if (r.tables)
            r.tables.forEach((t) =>
                console.log(
                    `    - ${t.id} rows=${t.rowCount} cols=[${t.headers.slice(0, 8).join(', ')}]`
                )
            );
    }
    ...
    for (const r of results) {
        ...
        if (r.forms?.length)
            md += `- Forms: ${r.forms.map((f) => `\`${f.id}\` (${f.method})`).join(', ')}\n`;
        if (r.tables?.length) {
            md += `- Tables:\n`;
            r.tables.forEach((t) => {
        ...
        if (r.buttons?.length) {
            ...
            r.buttons
                .slice(0, 20)
                .forEach(
                    (b) => (md += `  - \`${b.id}\`: ${b.text}${b.disabled ? ' (disabled)' : ''}\n`)
                );
        }
