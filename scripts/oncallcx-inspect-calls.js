...
(async () => {
    ...
    page.on('response', async r => {
        ...
        if (/audio|wav|mp3/.test(ct) || /\.(wav|mp3|ogg|m4a)(\?|$)/i.test(r.url())) {
            audio.push({ url: r.url(), status: r.status(), ct, size: r.headers()['content-length'] });
        }
    });
    ...
    const tables = await page.evaluate(() => {
        ...
        return tbls.map(t => ({
            ...
            headers: [...t.querySelectorAll('th, .ui-column-title')].slice(0, 15).map(x => x.textContent.trim().slice(0, 30)),
        }));
    });
    console.log('\n=== TABLES ===');
    tables.forEach(t => console.log(' ', t.id, '| rows:', t.rows, '| headers:', t.headers.join(' | ')));
    ...
    const buttons = await page.evaluate(() => {
        ...
        document.querySelectorAll('button, a, [role="button"], .ui-button').forEach(el => {
            ...
            if (/play|download|record|audio|listen/i.test(text + ' ' + title + ' ' + icon + ' ' + onclick)) {
    ...
    console.log('\n=== CALL ACTION BUTTONS ===');
    buttons.slice(0, 30).forEach(b => console.log(` text="${b.text}" title="${b.title}" icon="${b.icon}"\n   onclick=${b.onclick.slice(0, 150)}`));
    ...
    const playBtns = page.locator('[class*="play"], button:has-text("Play"), [title*="play" i], i.pi-play, i.fa-play');
    ...
    if (nPlay > 0) {
        for (let i = 0; i < Math.min(nPlay, 3); i++) {
            ...
            } catch (e) { console.log(`[err] play #${i}: ${e.message.slice(0, 80)}`); }
        }
    }
    await page.waitForTimeout(2000);
    console.log('\n=== AUDIO SAU CLICK ===');
