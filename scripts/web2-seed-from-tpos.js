...
const ENTITIES = [
    ...
    },
    {
        ...
        // ~3107 templates. GetViewV2 returns 29 base fields per row.
        tposPath: '/odata/ProductTemplate/ODataService.GetViewV2?%24orderby=DateCreated+desc',
        bulk: true,
        mapper: (r) => ({
            code: r.DefaultCode || `tpos-${r.Id}`,
            name: r.Name || r.NameGet || `Template ${r.Id}`,
            isActive: r.Active !== false,
            data: pickFields(r, [
                'Id',
                'Name',
    ...
    },
    {
        ...
        mapper: (r) => ({
            code: r.DefaultCode || `tpos-${r.Id}`,
            name: r.NameGet || r.Name || `Variant ${r.Id}`,
            isActive: r.Active !== false,
            data: pickFields(r, [
                'Id',
                'Name',
    ...
    },
    {
        ...
        bulk: true,
        mapper: (r) => ({
            code: r.Number || `tpos-${r.Id}`,
            name: r.PartnerDisplayName || r.FacebookName || `Order ${r.Number || r.Id}`,
            isActive: r.State !== 'cancel',
            data: pickFields(r, [
                'Id',
                'Number',
                'Type',
    ...
    },
    {
        ...
        mapper: (r) => ({
            code: r.Number || `tpos-${r.Id}`,
            name: r.PartnerDisplayName || `PO ${r.Number || r.Id}`,
            isActive: r.State !== 'cancel',
            data: pickFields(r, [
                'Id',
                'Number',
    ...
    },
    {
        ...
        bulk: true,
        mapper: (r) => ({
            // SaleOnline_Order Id is GUID — use Code (260501361 style) as code
            code: r.Code ? `${r.Code}` : `tpos-${r.Id}`,
            name: r.Name || r.Facebook_UserName || `Order ${r.Code || r.Id}`,
            isActive: r.Status !== 'Đã hủy',
            data: pickFields(r, [
                'Id',
                'Code',
                'ReversedCode',
    ...
    {
        slug: 'accounttax',
        tposPath: '/odata/AccountTax',
        mapper: (r) => ({
            code: r.Code || `tpos-${r.Id}`,
            name: r.Name || `Tax ${r.Id}`,
            isActive: r.Active !== false,
            data: pickFields(r, [
                'Id',
                'Name',
    ...
    {
        slug: 'stockwarehouse',
        tposPath: '/odata/StockWarehouse',
        mapper: (r) => ({
            code: r.Code || `tpos-${r.Id}`,
            name: r.Name || `Warehouse ${r.Id}`,
            isActive: r.Active !== false,
            data: pickFields(r, [
                'Id',
                'Name',
    ...
    {
        slug: 'crmteam',
        // CRMTeam.GetAllFacebook trả tree (Childs[]). Flatten parent + children → 1 array.
        tposPath: '/odata/CRMTeam/ODataService.GetAllFacebook?%24expand=Childs',
        bulk: false, // small, single-row OK
        mapper: (r) => ({
            code: `tpos-${r.Id}`,
    ...
    },
    {
        slug: 'livecampaign',
        tposPath:
            '/odata/SaleOnline_LiveCampaign/ODataService.GetAvailables?%24orderby=DateCreated+desc',
        mapper: (r) => ({
            code: `tpos-${r.Id || r.Code}`,
