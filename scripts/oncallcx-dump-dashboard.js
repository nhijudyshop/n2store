...
(async () => {
    ...
    const menu = await page.evaluate(() => {
        ...
        document.querySelectorAll('a, [role="menuitem"], li.menuitem, li[data-label], .ui-menuitem').forEach(el => {
            const onclick = el.getAttribute('onclick') || '';
            const href = el.getAttribute('href') || '';
            const dataUrl = el.getAttribute('data-url') || '';
            const text = (el.textContent || '').trim().replace(/\s+/g, ' ').slice(0, 120);
            if ((onclick || href || dataUrl) && text) {
                items.push({ text, href, onclick: onclick.slice(0, 300), dataUrl, cls: el.className });
            }
        });
        return items;
    });

    fs.writeFileSync(path.resolve(__dirname, '..', 'docs', 'oncallcx-menu.json'), JSON.stringify(menu, null, 2));

    // Filter những item liên quan
    const keywords = /call|record|audio|voicemail|cdr|history|log|report|extension|lịch sử|ghi âm|cuộc gọi|báo cáo/i;
    const filtered = menu.filter(m => keywords.test(m.text) || keywords.test(m.onclick) || keywords.test(m.href) || keywords.test(m.dataUrl));
    console.log('=== MENU ITEMS LIÊN QUAN ===');
    filtered.forEach(m => console.log(`  ${m.text}\n    href=${m.href}\n    onclick=${m.onclick.slice(0, 200)}\n`));
