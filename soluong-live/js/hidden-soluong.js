    ...
    try {
        ...
        if (settings) {
            displaySettings = {
                ...
                nameMargin: 3,
                itemsPerPage:
                    settings.itemsPerPage || (settings.columns || 4) * (settings.rows || 2),
            };
            itemsPerPage = displaySettings.itemsPerPage;
            console.log(
                '🔥 Loaded hidden products display settings from Firebase:',
                displaySettings
            );
        } else {
            displaySettings = {
...
function applySettings() {
    ...
    console.log(
        `✅ Applied settings: ${displaySettings.columns} columns x ${displaySettings.rows} rows, gap: ${displaySettings.gap}px`
    );
}
...
function performSearch(keyword, syncToFirebase = true) {
    ...
    const productsToSearch = isMergeVariants
        ? mergeProductsByTemplate(hiddenProducts)
        : hiddenProducts;
    ...
    syncPageListenerRef = database.ref('hiddenSoluongSyncCurrentPage').on('value', (snapshot) => {
        ...
        if (isSyncMode && page && !isSyncingFromFirebase && page !== currentPage) {
            ...
            const totalPages = Math.ceil(
                Object.keys(soluongProducts).filter((key) => soluongProducts[key].isHidden).length /
                    itemsPerPage
            );
...
window.addEventListener('hashchange', () => {
    ...
    if (isSyncMode && !wasSyncMode) {
        ...
        setupSyncListeners();
        database
            .ref('hiddenSoluongSyncCurrentPage')
            .set(currentPage)
            .catch((error) => {
