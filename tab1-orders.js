...
async function handleFileUpload(input) {
    ...
    sendBtn.disabled = true;
    sendBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Đang gửi file...';
...
async function handleMicClick() {
    ...
    if (mediaRecorder && mediaRecorder.state === 'recording') {
        mediaRecorder.stop();
        micBtn.style.color = '#595959';
        micBtn.classList.remove('recording-animation'); // Remove animation class if used
        return;
    }
    ...
    try {
        ...
        mediaRecorder.onstop = async () => {
            const audioBlob = new Blob(audioChunks, { type: 'audio/webm' }); // or audio/mp3 if supported/converted
        ...
        mediaRecorder.start();
        micBtn.style.color = '#ff4d4f'; // Red color indicating recording
...
async function uploadAndSendFile(file) {
    if (!currentOrder || !currentConversationId || !currentChatChannelId) {
        console.error('[UPLOAD] Missing context info');
        return;
    }

    const pageId = currentChatChannelId;
    const conversationId = currentConversationId;
    const token = await window.pancakeTokenManager.getToken(); // Use pancakeTokenManager

    // Optimistic UI Update (File)
    const now = new Date().toISOString();
    ...
    if (isImage) {
        ...
        messageContent = `<img src="${objectUrl}" style="max-width: 200px; border-radius: 8px;">`;
    } else if (isAudio) {
        messageContent = `[Audio] Đang gửi...`;
    } else {
    ...
    if (currentChatType === 'message') {
        ...
        // For comments, structure might be different, but let's try
        // allChatComments.push(...) 
    }
    ...
    if (currentChatType === 'message') {
        ...
        formData.append('message_id', currentParentCommentId);
        formData.append('parent_id', currentParentCommentId);
        formData.append('post_id', currentPostId);
    }

    formData.append('message', ''); // Empty message for file
    formData.append('send_by_platform', 'web');

    // Append file. Try 'attachments[]' as it's common for arrays, or 'file'
    // Based on typical Pancake behavior, it might be 'attachments' or 'file'
    // Let's try 'attachments[]' first as it supports multiple
    ...
    try {
        // Use fetch directly to handle FormData (smartFetch might force JSON)
        ...
        setTimeout(async () => {
            if (currentChatType === 'message') {
                const res = await window.chatDataManager.fetchMessages(currentChatChannelId, currentChatPSID);
                if (res.messages) {
                    allChatMessages = res.messages;
                    renderChatMessages(allChatMessages, true);
                }
            } else {
                const res = await window.chatDataManager.fetchComments(currentChatChannelId, currentChatPSID);
                if (res.comments) {
                    allChatComments = res.comments;
                    renderComments(allChatComments, true);
                }
            }
        }, 1000);
