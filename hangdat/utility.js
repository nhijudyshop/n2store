...
const productClipboardContainer = document.getElementById(
    "productClipboardContainer",
);
const priceClipboardContainer = document.getElementById(
    "priceClipboardContainer",
);
...
async function handleProductImageUpload(orderData) {
    const selectedType = document.querySelector(
        'input[name="productInputType"]:checked',
    ).value;
    ...
    if (selectedType === "link") {
        ...
        if (window.isProductUrlPasted && window.pastedProductImageUrl) {
            ...
            return await handleClipboardImageUpload(
                productImgArray[0],
                "product",
            );
        } else {
...
async function handlePriceImageUpload(orderData) {
    const selectedType = document.querySelector(
        'input[name="priceInputType"]:checked',
    ).value;
