...
function renderProductRow(opts) {
    ...
    return `
        <tr class="${rowClass} ${doneClass}"${dragAttrs}>
            ...
            <td class="col-colors editable-cell ${borderClass}${variantMismatch ? ' variant-mismatch-cell' : ''}" ${editAttrs} ondblclick="window.openVariantModal(this)"${variantMismatch ? mismatchTitle : ' title="Nhấp đúp để tạo biến thể"'}>${colorDetails}${variantMismatch ? ` <span class="variant-mismatch-badge" title="Tổng biến thể ${_sumVariants} ≠ Tổng SL ${_declaredTotal}">⚠ ${_sumVariants}≠${_declaredTotal}</span>` : ''}<button class="btn-edit-cell btn-edit-variant" onclick="event.stopPropagation(); window.openVariantModal(this.closest('td'))" title="Chọn biến thể"><i data-lucide="pencil"></i></button></td>
            <td class="col-qty text-center editable-cell ${borderClass}${variantMismatch ? ' variant-mismatch-cell' : ''}" ${editAttrs} data-field="tongSoLuong" ondblclick="startInlineEdit(this)"${variantMismatch ? mismatchTitle : ' title="Nhấp đúp để sửa"'}>${tongSoLuong !== '-' ? formatNumber(tongSoLuong) : '-'}<button class="btn-edit-cell btn-edit-qty" onclick="event.stopPropagation(); startInlineEdit(this.closest('td'))" title="Sửa Tổng SL"><i data-lucide="pencil"></i></button></td>
            <td class="col-price text-right editable-cell ${borderClass}" ${editAttrs} data-ti-gia="${tg}" data-field="giaDonVi" ondblclick="startInlineEdit(this)" title="Nhấp đúp để sửa (Đơn giá tiền Trung) — VND hiển thị trong ngoặc">${giaDonVi > 0 ? `${formatNumber(giaDonVi)}${_vndSuffixHtml(giaDonVi, tg)}` : '-'}<button class="btn-edit-cell btn-edit-price" onclick="event.stopPropagation(); startInlineEdit(this.closest('td'))" title="Sửa Đơn giá"><i data-lucide="pencil"></i></button></td>
            ${
                isFirstRow
                    ? `
