        ...
        .then((doc) => {
            if (doc.exists) {
                ...
                if (!Array.isArray(data["data"])) {
                    console.error(
                        "Lỗi: data['data'] không phải là một mảng hoặc chưa được khởi tạo.",
                        data,
                    );
...
function applyFiltersToData(dataArray) {
    const channelFilter = document.getElementById("channelFilter")
        ? document.getElementById("channelFilter").value.trim().toLowerCase()
        : "";
    const scenarioFilter = document.getElementById("scenarioFilter")
        ? document.getElementById("scenarioFilter").value.trim().toLowerCase()
        : "";
    ...
    return dataArray.filter((item) => {
        // Channel filter
        const channelText = item.shipValue
            ? item.shipValue.trim().toLowerCase()
            : "";
        const channelMatch =
            channelFilter === "all" ||
            channelText === channelFilter ||
            channelFilter === "";

        // Scenario filter
        const scenarioText = item.scenarioValue
            ? item.scenarioValue.trim().toLowerCase()
            : "";
        const scenarioMatch =
            scenarioFilter === "all" ||
            scenarioText === scenarioFilter ||
            scenarioFilter === "";
        ...
        const timestampdateText = parseDateText(
            formattedTime.replace(/\//g, "-"),
        );

        const dateMatch =
            !isNaN(timestampdateText) &&
            ((timestampstartDate === null && timestampendDate === null) ||
                (timestampstartDate !== null &&
                    timestampendDate !== null &&
                    timestampdateText >= timestampstartDate &&
                    timestampdateText <= timestampendDate));
    ...
    tableBody.addEventListener("change", function (event) {
        ...
        if (target.classList.contains("received-checkbox")) {
            ...
            if (target.checked) {
                ...
                const firstUncheckedRow = [...tableBody.rows].find(
                    (r) => !r.querySelector(".received-checkbox").checked,
                );
...
function handleCheckboxClick(e) {
    ...
    collectionRef
        ...
        .then(() => {
            ...
            logAction(
                "update",
                `${actionDesc}: ${updateData.customerInfoValue}`,
                updateData,
                newUpdateData,
            );
...
function initializeFilterEvents() {
    ...
    if (channelFilter)
        channelFilter.addEventListener("change", debouncedUpdateTable);
    if (scenarioFilter)
        scenarioFilter.addEventListener("change", debouncedUpdateTable);
