...
(function () {
    ...
    NO.bulkCreatePbhShop = async function bulkCreatePbhShop() {
        const codes = NO.getSelectedCodes();
        if (!codes.length) {
            NO.notify('Chưa chọn đơn nào', 'warning');
            return;
        ...
        const selOrders = codes
            .map((c) => NO.STATE.orders.find((x) => x.code === c))
            .filter(Boolean);
        ...
        if (valid.length === 1) {
            return NO.createPbh(valid[0], { shopMode: true });
        }
