...
# Phân tích chi tiết — OnCallCX UCaaS Hướng Dẫn Sử Dụng v1.1

> **Nguồn**: `orders-report/oncallcxucaasuserguidevieforcustomerv112 (2).pdf` (75 trang)
> **Nhà phát hành**: FPT Telecom — sản phẩm OnCallCX UCaaS
> **Phiên bản tài liệu**: 1.1
> **Ngày phân tích**: 2026-04-08

Tài liệu này phân tích chi tiết, có cấu trúc lại từng mục của PDF gốc, bổ sung bảng tra cứu nhanh, cảnh báo pitfall, và mối quan hệ phụ thuộc giữa các tính năng nhằm hỗ trợ admin PBX cấu hình hệ thống chính xác ngay lần đầu.
...
6. [anConnect & anMeet](#6-anconnect--anmeet)
7. [Thống kê cuộc gọi](#7-thống-kê-cuộc-gọi)
8. [OnCallCX cho MS Teams](#8-oncallcx-cho-ms-teams)
...
4. Xuất dữ liệu: **Export as .csv** / **Export as .xlsx**.
5. **Play Audio** / **Download** (nếu có recording).

---

## 8. ONCALLCX CHO MS TEAMS

### 8.1 Plugin OnCallCX trong Teams Client
...
### 8.2 Ứng dụng OnCallCX Desktop

**Quy trình provisioning**:
1. Admin PBX kích hoạt desktop client cho extension → user nhận email **"Desktop Application Provisioning"**.
2. Email chứa:
   - Link tải Windows (`.exe`)
   - Link tải macOS (`.pkg`)
   - **Activation link** dùng để kích hoạt
3. User cài đặt → mở **activation link từ chính máy đã cài** → app gắn với extension.

**Giao diện app desktop**:
- Dialpad (0–9, *, #)
- History tabs: `All` / `Outgoing` / `Missed` / `Incoming` / `Voicemail`
- Sidebar:
  - Portal (link sang Web Portal)
  - Receive calls toggle
  - Don't forward calls on busy
  - Don't forward calls
  - Settings
...
*Hết phân tích. Tài liệu này tóm lược 75 trang PDF gốc thành ~700 dòng markdown có cấu trúc. Tham khảo PDF gốc để xem screenshots chi tiết các bước thao tác UI.*
