    ...
    try {
        ...
        if (data.success && data.categorized?.activated) {
            ...
            if (pages.length > 0) {
                ...
                div.innerHTML = '<span style="color: #f59e0b;">⚠️ 0 pages (không có quyền trên page nào)</span>';
            }
        } else {
            div.innerHTML = '<span style="color: #f59e0b;">⚠️ Không thể kiểm tra pages</span>';
        }
    } catch (err) {
        console.error(`[PANCAKE-SETTINGS] Error fetching pages for ${accountId}:`, err);
        div.innerHTML = '<span style="color: #ef4444;">❌ Lỗi kiểm tra</span>';
    }
}
