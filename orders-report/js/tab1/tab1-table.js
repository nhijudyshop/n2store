...
window.addEventListener('failedOrdersUpdated', (event) => {
    ...
    document.querySelectorAll('td[data-column="comments"][data-order-id]').forEach(td => {
        ...
        if (isFailed && !currentlyShowingFailed) {
            ...
            // Mark as failed with quick fix button — escape orderId to prevent XSS
            const safeOrderId = String(orderId).replace(/['"\\<>&]/g, '');
            td.innerHTML = `
                <button onclick="window.messageTemplateManager?.openQuickCommentReply('${safeOrderId}')"
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <th data-column="invoice-status" style="min-width: 200px;">Phiếu bán hàng</th>
                            </tr>
                        </thead>
                        <tbody>
