...
(function () {
    ...
    function _userIdentifier() {
        try {
            const auth =
                window.authManager?.getAuthState?.() || window.authManager?.getAuthData?.();
            if (auth) {
                if (auth.userType) {
                    return auth.userType.includes('-')
    ...
    function _key() {
        return KEY_PREFIX + _userIdentifier();
    }
    ...
    async function _idbGet(key) {
        ...
        try {
            return await window.indexedDBStorage.getItem(key);
        } catch (e) {
            return null;
        }
    }
    ...
    function _collect() {
        ...
        const activeFlagFilters =
            ptagState._activeFlagFilters instanceof Set
                ? [...ptagState._activeFlagFilters]
                : Array.isArray(ptagState._activeFlagFilters)
                  ? ptagState._activeFlagFilters
                  : [];
    ...
    function _applySnapshot(snap) {
        ...
        try {
            ...
            if (s.query) {
                ...
                if (typeof window.searchQuery !== 'undefined') {
                    try {
                        window.searchQuery = String(s.query).toLowerCase();
                    } catch (e) {}
                }
            ...
            console.warn('[FILTER-PERSIST] apply search err:', e);
        }
        ...
        try {
            ...
            console.warn('[FILTER-PERSIST] apply tags err:', e);
        }
        ...
        try {
            ...
            if (Array.isArray(p.activeFlagFilters)) {
                localStorage.setItem(
                    'ptag_active_flag_filters_v1',
                    JSON.stringify(p.activeFlagFilters)
                );
            }
            ...
            console.warn('[FILTER-PERSIST] apply ptag err:', e);
        }
    ...
    async function _maybeCleanupLegacyLS(snap) {
        ...
        try {
            ...
            console.warn('[FILTER-PERSIST] LS cleanup error:', e);
        }
    }
        ...
        try {
            ...
            if (!snap) {
                ...
                if (!alreadyMigrated) {
                    ...
                    try {
                        localStorage.setItem(MIGRATION_MARKER_LS, String(Date.now()));
                    } catch (e) {}
                    console.log(
                        '[FILTER-PERSIST] Migrated legacy LS filters into IndexedDB (first account)'
                    );
                } else {
                    snap = {
                        v: 1,
                        savedAt: Date.now(),
                        migratedAt:
                            parseInt(localStorage.getItem(MIGRATION_MARKER_LS), 10) || Date.now(),
