    ...
    _setupListener() {
        ...
        try {
            this._unsubscribe = this._getDocRef().onSnapshot((doc) => {
                ...
                if (
                    window.currentConversationType === 'COMMENT' &&
                    window.allChatMessages.length > 0
                ) {
    ...
    const html = messages
        ...
        .map((msg) => {
            ...
            const isPrivateReply =
                isCommentConv && (!!msg.privateReplyConversation || PrivateReplyStore.has(msg.id));
            ...
            if (reactionSummary && typeof reactionSummary === 'object') {
                ...
                const parts = Object.entries(reactionSummary)
                    ...
                    .map(
                        ([type, count]) =>
                            `<span class="reaction-badge">${icons[type] || '👍'}${count > 1 ? ' ' + count : ''}</span>`
                    );
                if (parts.length > 0) {
            ...
            if (isRemoved) {
                statusIndicator =
                    '<span class="msg-status-indicator removed" title="Đã xóa"><i class="fas fa-trash-alt"></i></span>';
            } else if (isHidden) {
                statusIndicator =
                    '<span class="msg-status-indicator hidden-msg" title="Đã ẩn"><i class="fas fa-eye-slash"></i></span>';
            }
            ...
            if (isPrivateReply) {
                replyBadge =
                    '<span class="msg-reply-badge private"><i class="fas fa-lock"></i> Nhắn riêng</span>';
            }
...
function _buildMessageActions(msg, isCommentConv) {
    ...
    if (isCommentConv) {
        if (msg.canLike) {
            ...
            actions.push(
                `<button class="msg-action-btn${likeClass}" onclick="window.toggleLikeComment('${msg.id}')" title="${msg.userLikes ? 'Bỏ thích' : 'Thích'}"><i class="fas fa-thumbs-up"></i></button>`
            );
        }
        if (msg.canHide && msg.sender === 'customer') {
            ...
            actions.push(
                `<button class="msg-action-btn" onclick="window.toggleHideComment('${msg.id}')" title="${hideTitle}"><i class="fas ${hideIcon}"></i></button>`
            );
        }
        if (msg.canRemove) {
            actions.push(
                `<button class="msg-action-btn danger" onclick="window.deleteComment('${msg.id}')" title="Xóa bình luận"><i class="fas fa-trash-alt"></i></button>`
            );
        }
    }
...
window.sendMessage = async function () {
    ...
    if (
        ...
        window.currentReplyType === 'reply_comment'
    ) {
...
async function _sendInbox(pdm, pageId, convId, text, pat, replyData) {
    ...
    try {
        ...
        const reason = err.fbError?.is24HourError
            ? 'Quá 24h'
            : err.fbError?.isUserUnavailable
              ? 'Khách không có mặt (#551)'
        ...
        try {
            ...
            const psid = window.currentChatPSID;
            let convFromCache =
                pdm?.inboxMapByPSID?.get(String(psid)) || window.currentConversationData || {};
            console.log('convFromCache snapshot:', {
                ...
                customers_with_global_id: (convFromCache?.customers || []).filter(
                    (c) => c.global_id
                ).length,
                ...
                if (extErr.message?.includes('Global Facebook ID') && window.buildConvData) {
                    try {
                        ...
                        console.log('fallbackConv:', {
                            ...
                            raw_page_customer_global_id:
                                fallbackConv?._raw?.page_customer?.global_id,
...
async function _sendComment(pdm, pageId, convId, text, pat, replyData) {
    ...
    if (type === 'reply_comment') {
        ...
        try {
            ...
            const isPostGone =
                err.fbError?.eCode === 100 ||
                err.message?.includes('does not exist') ||
                err.message?.includes('does not support');
            if (isPostGone) {
                throw new Error(
                    'Bài viết/bình luận không còn tồn tại trên Facebook hoặc page không có quyền truy cập.'
                );
            }
            ...
            try {
                ...
                if (window.pancakeExtension?.connected && window.sendPrivateReplyViaExtension) {
                    try {
                        ...
                        console.warn(
                            '[Chat-Msg] Extension SEND_PRIVATE_REPLY failed:',
                            extErr2.message
                        );
                    }
                }
...
function _parseFbError(responseText) {
    try {
        ...
        const isUserUnavailable = eCode === 551 || (message && message.includes('không có mặt'));
...
function _updateDeletedBanner(messages) {
    ...
    if (allRemoved) {
        ...
        if (input) {
            input.disabled = true;
            input.placeholder = 'Không thể trả lời — bình luận đã bị xóa';
        }
        const sendBtn = document.querySelector('.send-btn');
        if (sendBtn) sendBtn.disabled = true;
    } else {
        ...
        if (input) {
            ...
            input.placeholder = 'Nhập tin nhắn... (/ để mở gợi ý)';
        }
    }
}
...
(function () {
    ...
    function renderGrid(category) {
        ...
        grid.innerHTML = emojiData[category]
