...
(function () {
    ...
    function renderUploadHistoryDetailV2(record) {
        ...
        if (Object.keys(productsByCode).length === 0) {
            ...
            html += `<div class="card mb-3 border-primary"><div class="card-body"><table class="table table-sm table-bordered"><thead class="table-light"><tr><th style="width: 50%;">Sản phẩm</th><th class="text-center" style="width: 15%;">Số lượng</th><th class="text-center" style="width: 20%;">Mã đơn hàng</th><th style="width: 15%;">Note</th></tr></thead><tbody>${Object.values(productsByCode).map(product => `<tr><td><div class="d-flex align-items-center gap-2">${product.imageUrl ? `<img src="${window.TPOSImageProxy ? window.TPOSImageProxy.proxyImageUrl(product.imageUrl) : product.imageUrl}" style="width: 40px; height: 40px; object-fit: cover; border-radius: 4px;">` : '<div style="width: 40px; height: 40px; background: #e5e7eb; border-radius: 4px; display: flex; align-items: center; justify-content: center;">📦</div>'}<div style="flex: 1;"><div style="font-weight: 600; font-size: 14px;">${product.productName}</div><div style="font-size: 12px; color: #6b7280;">${product.productCode || 'N/A'}</div></div></div></td><td class="text-center"><span class="badge bg-primary">${product.count}</span></td><td class="text-center small">${product.sttList.join(', ')}</td><td class="small">${product.note}</td></tr>`).join('')}</tbody></table></div></div>`;
        }
