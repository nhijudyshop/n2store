...
class PancakeTokenManager {
    ...
    getPageAccessTokensFromLocalStorage() {
        // If in-memory cache is empty, try to load from n2store synchronously
        if (!this.pageAccessTokens || Object.keys(this.pageAccessTokens).length === 0) {
            try {
                ...
                console.warn('[PANCAKE-TOKEN] Error reading from n2store:', error);
            }
        }
    ...
    async loadAccounts() {
        try {
            ...
            // Load active account ID from n2store (per-device)
