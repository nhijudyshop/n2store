...
class TokenManager {
    ...
    async init() {
        ...
        if (firestoreToken) {
            ...
            try {
                ...
                console.log('[TOKEN] ✅ Valid token loaded from Firestore and synced to n2store');
            } catch (error) {
                console.error('[TOKEN] Error syncing Firestore token to n2store:', error);
            }
            return;
        }
    ...
    loadFromStorage() {
        try {
            ...
            if (!stored) {
                console.log('[TOKEN] No token found in n2store');
                return;
            }
    ...
    async saveToStorage(tokenData) {
        try {
            ...
            console.log('[TOKEN] Token saved to n2store, expires:', new Date(expiresAt).toLocaleString());
    ...
    async clearToken() {
        ...
        if (this.firestoreRef) {
            try {
                ...
                console.log('[TOKEN] Token cleared from n2store and Firestore');
            } catch (error) {
                console.error('[TOKEN] Error clearing token from Firestore:', error);
            }
        } else {
            console.log('[TOKEN] Token cleared from n2store');
        }
    }
