...
// RESET DEFAULT TAGS
// =====================================================
async function resetDefaultTags() {
    const confirmed = confirm(
        'Bạn có chắc muốn reset danh sách tag về mặc định?\n\n' +
        'Danh sách tag mới sẽ bao gồm:\n' +
        '• ĐÃ RA ĐƠN (màu xanh dương)\n' +
        '• CHỜ HÀNG VỀ (màu cam)\n' +
        '• GIỎ TRỐNG (màu vàng)\n' +
        '• XÃ ĐƠN (màu đỏ)\n' +
        '• OK (màu xanh lá)\n' +
        ...
        '• CHỜ LIVE (màu hồng)\n' +
        '• GỘP (màu xám)\n\n' +
        'Lưu ý: Tag cũ sẽ bị xóa nhưng đơn hàng đã gán tag vẫn giữ nguyên.'
    );
    ...
    try {
        ...
        const defaultTags = [
            { Id: 'tag_da_ra_don', Name: 'ĐÃ RA ĐƠN', Color: '#3b82f6' },      // Blue - for Ordered status
            { Id: 'tag_cho_hang_ve', Name: 'CHỜ HÀNG VỀ', Color: '#f97316' },  // Orange
            { Id: 'tag_gio_trong', Name: 'GIỎ TRỐNG', Color: '#eab308' },      // Yellow
            { Id: 'tag_xa_don', Name: 'XÃ ĐƠN', Color: '#ef4444' },            // Red
            { Id: 'tag_ok', Name: 'OK', Color: '#10b981' },                     // Green
            { Id: 'tag_xu_ly', Name: 'XỬ LÝ', Color: '#8b5cf6' },              // Purple
            { Id: 'tag_cho_live', Name: 'CHỜ LIVE', Color: '#ec4899' },        // Pink
            { Id: 'tag_gop', Name: 'GỘP', Color: '#6b7280' }                    // Gray - merged orders not confirmed
        ];

        // Save to Firebase
        if (database) {
            await database.ref('settings/tags').set(defaultTags);
            console.log('[RESET-TAGS] Saved default tags to Firebase');
        }
        ...
        // Render tag list if modal is open
        const searchInput = document.getElementById("tagSearchInput");
        if (searchInput) {
            searchInput.value = "";
        }
        ...
        // Render tag settings list if settings modal is open
        if (typeof renderTagSettingsList === 'function') {
            renderTagSettingsList();
        }

        // Show success notification
        if (window.notificationManager) {
            window.notificationManager.success('Đã reset danh sách tag về mặc định thành công!', 4000);
        } else {
            alert('Đã reset danh sách tag về mặc định!');
        }
        ...
        console.error('[RESET-TAGS] Error resetting tags:', error);
        if (window.notificationManager) {
            window.notificationManager.error('Lỗi reset tag: ' + error.message);
        } else {
            alert('Lỗi reset tag: ' + error.message);
        }
    }
}
...
function updateStats() {
    ...
    // Update order stats badges
    updateOrderStatsBadges();
}

// =====================================================
// ORDER STATS BADGES CALCULATION
// =====================================================
function updateOrderStatsBadges() {
    // x = Total orders
    const x = filteredData.length;

    // Z = Orders with tag "GIỎ TRỐNG"
    let emptyCartCount = 0;
    // K = Orders with tag "GỘP" (merged orders not confirmed)
    let mergedTagCount = 0;
    // H = Total orders with tags EXCEPT "GIỎ TRỐNG" and "GỘP"
    let taggedOrdersCount = 0;

    const excludedTags = ["GIỎ TRỐNG", "GỘP"];
        ...
        if (order.Tags) {
            try {
                ...
                if (Array.isArray(tags) && tags.length > 0) {
                    ...
                    // Check for "GỘP" tag
                    const hasMergedTag = tags.some(tag =>
                        tag.Name && tag.Name.toUpperCase() === "GỘP"
                    );
                    if (hasMergedTag) {
                        mergedTagCount++;
                    }

                    // Check if has any tag EXCEPT excluded ones
                    const hasOtherTags = tags.some(tag =>
                        tag.Name && !excludedTags.includes(tag.Name.toUpperCase())
                    );
                    if (hasOtherTags) {
                        taggedOrdersCount++;
                    }
    ...
    // Y = x - Z - K (ĐƠN CHỐT = Total - Empty Cart - Merged)
    const y = x - emptyCartCount - mergedTagCount;

    // H = total orders with tags except "GIỎ TRỐNG" and "GỘP"
    const h = taggedOrdersCount;

    // Update badge values (Header badges - always visible)
    const statTotalOrders = document.getElementById("statTotalOrdersHeader");
    const statConfirmedOrders = document.getElementById("statConfirmedOrdersHeader");
    const statEmptyCart = document.getElementById("statEmptyCartHeader");
    const statMergedOrders = document.getElementById("statMergedOrdersHeader");
    const statValidationBadge = document.getElementById("statValidationBadgeHeader");

    if (statTotalOrders) statTotalOrders.textContent = x.toLocaleString("vi-VN");
    if (statConfirmedOrders) statConfirmedOrders.textContent = y.toLocaleString("vi-VN");
    if (statEmptyCart) statEmptyCart.textContent = emptyCartCount.toLocaleString("vi-VN");
    if (statMergedOrders) statMergedOrders.textContent = mergedTagCount.toLocaleString("vi-VN");

    // Update validation badge (Y == H check)
    if (statValidationBadge) {
        if (y === h) {
            // Green - Y equals H
            statValidationBadge.style.background = "linear-gradient(135deg, #10b981 0%, #059669 100%)";
            statValidationBadge.style.boxShadow = "0 2px 4px rgba(16, 185, 129, 0.3)";
            statValidationBadge.innerHTML = `<i class="fas fa-check-circle"></i> <span id="statValidationTextHeader">H = ${h}</span>`;
        } else {
            // Red - Y not equals H, need to check
            statValidationBadge.style.background = "linear-gradient(135deg, #ef4444 0%, #dc2626 100%)";
            statValidationBadge.style.boxShadow = "0 2px 4px rgba(239, 68, 68, 0.3)";
            statValidationBadge.innerHTML = `<i class="fas fa-exclamation-triangle"></i> <span id="statValidationTextHeader">Y=${y} ≠ H=${h} - KIỂM TRA LẠI</span>`;
        }
    }

    console.log(`[STATS] x=${x}, y=${y}, Z=${emptyCartCount}, K=${mergedTagCount}, H=${h}`);
}
