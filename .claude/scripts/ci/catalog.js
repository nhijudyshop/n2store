...
function buildCatalog() {
    const agents = listMatchingFiles(
        'agents',
        (entry) => entry.isFile() && entry.name.endsWith('.md')
    );
    const commands = listMatchingFiles(
        'commands',
        (entry) => entry.isFile() && entry.name.endsWith('.md')
    );
    const skills = listMatchingFiles(
        'skills',
        (entry) =>
...
function parseReadmeExpectations(readmeContent) {
    ...
    expectations.push(
        {
            category: 'agents',
            mode: 'exact',
            expected: Number(quickStartMatch[1]),
            source: 'README.md quick-start summary',
        },
        {
            category: 'skills',
            mode: 'exact',
            expected: Number(quickStartMatch[2]),
            source: 'README.md quick-start summary',
        },
        {
            category: 'commands',
            mode: 'exact',
            expected: Number(quickStartMatch[3]),
            source: 'README.md quick-start summary',
        }
    );
    ...
    const tablePatterns = [
        {
            category: 'agents',
            regex: /\|\s*(?:\*\*)?Agents(?:\*\*)?\s*\|\s*(?:(?:PASS:|\u2705)\s*)?(\d+)\s+agents\s*\|/i,
            source: 'README.md comparison table',
        },
        {
            category: 'commands',
            regex: /\|\s*(?:\*\*)?Commands(?:\*\*)?\s*\|\s*(?:(?:PASS:|\u2705)\s*)?(\d+)\s+commands\s*\|/i,
            source: 'README.md comparison table',
        },
        {
            category: 'skills',
            regex: /\|\s*(?:\*\*)?Skills(?:\*\*)?\s*\|\s*(?:(?:PASS:|\u2705)\s*)?(\d+)\s+skills\s*\|/i,
            source: 'README.md comparison table',
        },
    ];
    ...
    return [
        {
            ...
            mode: 'exact',
            expected: Number(match[1]),
            source: 'README.zh-CN.md quick-start summary',
        },
        {
            category: 'skills',
            mode: 'exact',
...
function parseZhDocsReadmeExpectations(readmeContent) {
    ...
    expectations.push(
        {
            ...
            mode: 'exact',
            expected: Number(quickStartMatch[1]),
            source: 'docs/zh-CN/README.md quick-start summary',
        },
        {
            category: 'skills',
            mode: 'exact',
    ...
    const tablePatterns = [
        {
            category: 'agents',
            regex: /\|\s*智能体\s*\|\s*(?:(?:PASS:|\u2705)\s*)?(\d+)\s*个\s*\|/i,
            source: 'docs/zh-CN/README.md comparison table',
        },
        {
            category: 'commands',
            regex: /\|\s*命令\s*\|\s*(?:(?:PASS:|\u2705)\s*)?(\d+)\s*个\s*\|/i,
            source: 'docs/zh-CN/README.md comparison table',
        },
        {
            category: 'skills',
            regex: /\|\s*技能\s*\|\s*(?:(?:PASS:|\u2705)\s*)?(\d+)\s*项\s*\|/i,
            source: 'docs/zh-CN/README.md comparison table',
        },
    ];
...
function parseAgentsDocExpectations(agentsContent) {
    const summaryMatch = agentsContent.match(
    ...
    const expectations = [
        {
            category: 'agents',
            mode: 'exact',
            expected: Number(summaryMatch[1]),
        ...
        },
        {
            category: 'commands',
            mode: 'exact',
            expected: Number(summaryMatch[4]),
            source: 'AGENTS.md summary',
        },
    ];
...
function renderMarkdown(result) {
    ...
    console.log(
        `| Commands | ${result.catalog.commands.count} | \`${result.catalog.commands.glob}\` |`
    );
