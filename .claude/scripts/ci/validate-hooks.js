    ...
    if (fs.existsSync(HOOKS_SCHEMA_PATH)) {
        ...
        if (!valid) {
            for (const err of validate.errors) {
                console.error(
                    `ERROR: hooks.json schema: ${err.instancePath || '/'} ${err.message}`
                );
            }
            process.exit(1);
        }
    ...
    if (typeof hooks === 'object' && !Array.isArray(hooks)) {
        ...
        for (const [eventType, matchers] of Object.entries(hooks)) {
            ...
            for (let i = 0; i < matchers.length; i++) {
                ...
                if (!('matcher' in matcher) && !EVENTS_WITHOUT_MATCHER.has(eventType)) {
                    ...
                    (typeof matcher.matcher !== 'object' || matcher.matcher === null)
                ) {
        ...
        for (let i = 0; i < hooks.length; i++) {
            ...
            if (!('matcher' in hook)) {
                ...
                (typeof hook.matcher !== 'object' || hook.matcher === null)
            ) {
