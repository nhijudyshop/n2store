...
function validateInstallManifests() {
    ...
    hasErrors =
        validateSchema(ajv, PROFILES_SCHEMA_PATH, profilesData, 'install-profiles.json') ||
        hasErrors;
    if (fs.existsSync(COMPONENTS_MANIFEST_PATH)) {
        hasErrors =
            validateSchema(
    ...
    for (const module of modules) {
        ...
        for (const dependency of module.dependencies) {
            if (
                ...
                !modules.some((candidate) => candidate.id === dependency)
            ) {
    ...
    for (const component of components) {
        ...
        for (const moduleId of component.modules) {
            if (!moduleIds.has(moduleId)) {
                console.error(
                    `ERROR: Component ${component.id} references unknown module ${moduleId}`
                );
                hasErrors = true;
            }
            ...
            if (seenModules.has(moduleId)) {
                console.error(
                    `ERROR: Component ${component.id} contains duplicate module ${moduleId}`
                );
                hasErrors = true;
            }
            seenModules.add(moduleId);
