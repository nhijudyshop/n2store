...
const HEALTHY_HTTP_CODES = new Set([
    200, 201, 202, 204, 301, 302, 303, 304, 307, 308, 400, 401, 403, 405,
]);
    ...
    return new Promise((resolve) => {
        ...
        child.on('exit', (code, signal) => {
            finish({
                ...
                reason:
                    stderr.trim() ||
                    `process exited before handshake (${signal || code || 'unknown'})`,
            });
        });
...
async function main() {
    ...
    const target =
        extractMcpTarget(input) || (truncated ? extractMcpTargetFromRaw(rawInput) : null);
