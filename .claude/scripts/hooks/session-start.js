...
function readInstinctsFromDir(directory, scope) {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            log(
                `[SessionStart] Warning: failed to parse instinct file ${filePath}: ${error.message}`
            );
        }
    }
...
function summarizeActiveInstincts(observerContext) {
    ...
    const projectDirs = observerContext.isGlobal
        ...
        : [
              {
                  dir: path.join(observerContext.projectDir, 'instincts', 'personal'),
                  scope: 'project',
              },
              {
                  dir: path.join(observerContext.projectDir, 'instincts', 'inherited'),
                  scope: 'project',
              },
          ];
    ...
    for (const instinct of scopedInstincts) {
        ...
        if (
            !existing ||
            (existing._scopeLabel !== 'project' && instinct._scopeLabel === 'project')
        ) {
    ...
    const ranked = Array.from(deduped.values())
        ...
        .sort((left, right) => {
            ...
            if (left._scopeLabel !== right._scopeLabel)
                return left._scopeLabel === 'project' ? -1 : 1;
