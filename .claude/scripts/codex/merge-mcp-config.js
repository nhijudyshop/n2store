...
const ECC_SERVERS = {
    supabase: dlxServer(
        ...
        { startup_timeout_sec: 20.0, tool_timeout_sec: 120.0 },
        'startup_timeout_sec = 20.0\ntool_timeout_sec = 120.0'
    ),
    playwright: dlxServer(
        'playwright',
...
ECC_SERVERS.supabase.fields.args.push(
    '--features=account,docs,database,debugging,development,functions,storage,branching'
);
ECC_SERVERS.supabase.toml = ECC_SERVERS.supabase.toml.replace(
    /^(args = \[.*)\]$/m,
    '$1, "--features=account,docs,database,debugging,development,functions,storage,branching"]'
);
...
function main() {
    ...
    for (const [name, spec] of Object.entries(ECC_SERVERS)) {
        ...
        const legacyName = aliases.find(
            (a) => existing[a] && typeof existing[a].command === 'string'
        );
        ...
        if (finalEntry) {
            if (updateMcp) {
                ...
                if (legacyName && !hasCanonical) {
                    ...
                    warn(
                        `mcp_servers.${name} differs from ECC recommendation (run with --update-mcp to refresh)`
                    );
                } else {
