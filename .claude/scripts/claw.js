...
function loadECCContext(skillList) {
    const requested = normalizeSkillList(
        skillList !== undefined ? skillList : process.env.CLAW_SKILLS || ''
    );
...
function compactSession(filePath, keepTurns = DEFAULT_COMPACT_KEEP_TURNS) {
    ...
    const compactedTurns = retained
        .map((t) => `### [${t.timestamp}] ${t.role}\n${t.content}\n---\n`)
        .join('');
...
function main() {
    ...
    if (!isValidSessionName(initialSessionName)) {
        console.error(
            `Error: Invalid session name "${initialSessionName}". Use alphanumeric characters and hyphens only.`
        );
        process.exit(1);
    }
    ...
    const prompt = () => {
        rl.question('claw> ', (input) => {
            ...
            if (line === '/metrics') {
                ...
                console.log(`Model: ${state.model}`);
                console.log(
                    `Turns: ${m.turns} (user ${m.userTurns}, assistant ${m.assistantTurns})`
                );
