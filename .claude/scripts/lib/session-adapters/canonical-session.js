...
function summarizeRawWorkerStates(snapshot) {
    ...
    return (snapshot.workers || []).reduce((counts, worker) => {
        const state =
            worker && worker.status && worker.status.state ? worker.status.state : 'unknown';
...
function validateCanonicalSnapshot(snapshot) {
    ...
    snapshot.workers.forEach((worker, index) => {
        ...
        ensureArrayOfStrings(
            worker.outputs.remainingRisks,
            `workers[${index}].outputs.remainingRisks`
        );
...
function writeFallbackSessionRecording(snapshot, options = {}) {
    ...
    const snapshotChanged =
        !existing || JSON.stringify(existing.latest) !== JSON.stringify(snapshot);
    ...
    const payload = {
        ...
        createdAt:
            existing && typeof existing.createdAt === 'string' ? existing.createdAt : recordedAt,
