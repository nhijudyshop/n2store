...
function rollbackCreatedResources(plan, createdState, runtime = {}) {
    ...
    if (createdState.sessionCreated) {
        try {
            runCommandImpl('tmux', ['kill-session', '-t', plan.sessionName], {
                cwd: plan.repoRoot,
            });
        } catch (error) {
    ...
    for (const workerPlan of [...createdState.workerPlans].reverse()) {
        ...
        if (existingWorktree) {
            try {
                runCommandImpl(
                    'git',
                    ['worktree', 'remove', '--force', existingWorktree.listedPath],
                    {
                        cwd: plan.repoRoot,
                    }
                );
        ...
        if (branchExistsImpl(plan.repoRoot, workerPlan.branchName)) {
            try {
                runCommandImpl('git', ['branch', '-D', workerPlan.branchName], {
                    cwd: plan.repoRoot,
                });
            } catch (error) {
                errors.push(error.message);
...
function executePlan(plan, runtime = {}) {
    ...
    const rollbackCreatedResourcesImpl =
        runtime.rollbackCreatedResources || rollbackCreatedResources;
    const createdState = {
        workerPlans: [],
