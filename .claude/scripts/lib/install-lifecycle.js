    ...
    if (operationHealth.missingSource.length > 0) {
        issues.push(
            buildIssue(
                ...
                {
                    paths: operationHealth.missingSource
    ...
    if (operationHealth.unverified.length > 0) {
        issues.push(
            buildIssue(
                ...
                {
                    paths: operationHealth.unverified
...
function repairInstalledStates(options = {}) {
    ...
    const summary = results.reduce(
        (accumulator, result) => ({
            ...
            plannedRepairCount:
                accumulator.plannedRepairCount + (result.status === 'planned' ? 1 : 0),
...
function uninstallInstalledStates(options = {}) {
    ...
    const summary = results.reduce(
        (accumulator, result) => ({
            checkedCount: accumulator.checkedCount + 1,
            uninstalledCount:
                accumulator.uninstalledCount + (result.status === 'uninstalled' ? 1 : 0),
            plannedRemovalCount:
                accumulator.plannedRemovalCount + (result.status === 'planned' ? 1 : 0),
            errorCount: accumulator.errorCount + (result.status === 'error' ? 1 : 0),
        }),
        {
            checkedCount: 0,
