    ...
    for (let i = 0; i < args.length; i++) {
        switch (args[i]) {
            ...
            case '--tailwind-config':
                opts.tailwindConfig = args[++i];
                break;
            case '--no-tailwind':
                opts.noTailwind = true;
                break;
            case '--index-css':
                opts.indexCss = args[++i];
                break;
            case '--css-files':
                opts.cssFiles.push(args[++i]);
                break;
            case '--extra-css':
                opts.extraCss = args[++i];
                break;
            case '--html-class':
...
function fetchAndEncode(url: string, timeout: number, redirectCount = 0): Promise<string> {
    ...
    if (redirectCount >= MAX_REDIRECTS) {
        ...
        console.warn(
            `  WARN: Too many redirects (${MAX_REDIRECTS}) <- ${url.slice(0, 70).replace(/\n|\r/g, '')}...`
        );
    ...
    return new Promise((resolve) => {
        ...
        if (!isSafeUrl(parsedUrl)) {
            const fallback =
                'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
            console.warn(
                `  WARN: Blocked request to non-public URL: ${url.slice(0, 70).replace(/\n|\r/g, '')}...`
            );
            imgCache.set(url, fallback);
            resolve(fallback);
            return;
            ...
            (resp) => {
                ...
                resp.on('error', (e: Error) => {
                    const fallback =
                        'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
                    console.warn(
                        `  WARN: Stream error: ${e.message.replace(/\n|\r/g, '')} <- ${url.slice(0, 70).replace(/\n|\r/g, '')}...`
                    );
                    imgCache.set(url, fallback);
                    resolve(fallback);
                });
        ...
        req.on('timeout', () => {
            ...
            const fallback =
                'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
            console.warn(
                `  WARN: Timeout after ${timeout}ms <- ${url.slice(0, 70).replace(/\n|\r/g, '')}...`
            );
            imgCache.set(url, fallback);
            resolve(fallback);
        });
...
async function embedImages(html: string, concurrency: number, timeout: number): Promise<string> {
    ...
    await Promise.all(srcImageMatches.map((m) => limit(() => fetchAndEncode(m[1], timeout))));
    ...
    const httpUrlRefs = cssUrlRefs.filter(
        (ref) =>
            (ref.url.startsWith('http://') || ref.url.startsWith('https://')) && isImageUrl(ref.url)
    );

    // Prefetch all CSS url() references concurrently
    await Promise.all(httpUrlRefs.map((ref) => limit(() => fetchAndEncode(ref.url, timeout))));
    ...
    await Promise.all(posterMatches.map((m) => limit(() => fetchAndEncode(m[1], timeout))));
...
function renderExpression(expr: any): string {
    ...
    switch (expr.type) {
        ...
        default:
            console.warn(
                `  WARN: Unhandled JSX expression of type "${expr.type}" inside child node.`
            );
            return '';
    }
}
    ...
    for (const attr of attrs) {
        ...
        if (attr.value.type === 'StringLiteral') {
            ...
            if (name === 'style' && expr.type === 'ObjectExpression') {
                ...
                console.warn(
                    `  WARN: Unhandled JSX expression of type "${expr.type}" inside attribute "${name}".`
                );
            }
        }
    }
...
function renderStyleObject(objExpr: any): string {
    ...
    for (const prop of objExpr.properties) {
        ...
        if (prop.value.type === 'StringLiteral') val = prop.value.value;
            ...
            val = prop.value.value === 0 ? '0' : `${prop.value.value}px`;
        else if (prop.value.type === 'TemplateLiteral')
            val = prop.value.quasis.map((q: any) => q.value.raw).join('');
        else continue;
...
function readCssFile(filePath: string | null): {
    imports: string[];
    css: string;
    hasApply: boolean;
} {
...
function extractFromHtml(htmlPath: string | null): { styles: string; links: string[] } {
    ...
    for (const m of html.matchAll(
        /<link[^>]+href="([^"]+)"[^>]*rel="stylesheet"[^>]*\/?>|<link[^>]+rel="stylesheet"[^>]*href="([^"]+)"[^>]*\/?>/g
    )) {
        const href = m[1] || m[2];
...
function buildHead(opts: Opts): string {
    ...
    if (useTailwind && !tailwindConfig) {
        ...
        if (tailwindConfig) console.log(`Auto-detected Tailwind config: ${tailwindConfig}`);
            ...
            useTailwind = false;
            console.log('No Tailwind config found — skipping CDN.');
        }
    }
...
async function main(): Promise<void> {
    ...
    for (const spec of opts.pages) {
        ...
        if (outerMatch) {
            ...
            fullHtml = head.replace('{{title}}', title) + `<body>\n${body}\n</body></html>\n`;
        }
