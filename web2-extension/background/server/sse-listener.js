...
function connect(url) {
    ...
    eventSource.addEventListener('new-transaction', (e) => {
        try {
            ...
            showNotification('new_transaction', `${amount} tu ${bank}\n${content}`, {
                contextMessage: data.transactionDate || '',
                raw: data,
            });
        } catch (err) {
    ...
    eventSource.addEventListener('update', (e) => {
        try {
            ...
            if (data.key && data.key.startsWith('held_products')) {
                ...
                showNotification('held_product', `${product} dang bi hold boi ${user}`, {
                    raw: data,
                });
                ...
                showNotification('new_message', `${from}: ${msg}`, { raw: data });
