...
export async function handleSendComment(data, sendResponse) {
    ...
    log.info(
        MODULE,
        `Sending comment: page=${pageId}, post=${postId}, reply_to=${commentId || 'root'}`
    );

    if (!pageId)
        return sendResponse({ type: 'SEND_COMMENT_FAILURE', taskId, error: 'pageId required' });
    if (!postId && !commentId)
        return sendResponse({
    ...
    if (!message)
        return sendResponse({ type: 'SEND_COMMENT_FAILURE', taskId, error: 'message required' });
    ...
    try {
        ...
        log.info(
            MODULE,
            `[DEBUG] Comment response (${text.length} chars): ${text.substring(0, 300)}`
        );
        ...
        if (result.error) {
            throw new Error(
                result.error.message || result.error.summary || 'Facebook comment error'
            );
        }

        // Extract comment ID from response
        const newCommentId =
            result?.payload?.comment_id ||
            result?.payload?.id ||
...
export async function handleSendPrivateReply(data, sendResponse) {
    ...
    try {
        ...
        log.info(
            MODULE,
            `[DEBUG] POST ${CONFIG.FB_GRAPHQL} (PagesManagerInboxPrivateReplyMutation)`
        );
        ...
        log.info(
            MODULE,
            `[DEBUG] Private reply response (${text.length} chars): ${text.substring(0, 300)}`
        );
