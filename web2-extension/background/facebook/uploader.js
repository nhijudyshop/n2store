...
async function uploadToFacebook(blob, filename, session, pageId) {
    ...
    log.info(
        MODULE,
        `[DEBUG] Upload response: status=${response.status}, type=${response.headers.get('content-type')}`
    );

    const text = await response.text();
    log.info(
        MODULE,
        `[DEBUG] Upload response body (${text.length} chars): ${text.substring(0, 500)}`
    );
