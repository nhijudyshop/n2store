...
export async function handleGetGlobalIdForConv(data, sendResponse) {
    ...
    log.info(
        MODULE,
        `Resolving global ID: page=${pageId}, thread=${threadId}, name=${customerName || '?'}`
    );
...
async function resolveGlobalId(session, pageId, threadId, isBusiness, extra = {}) {
    ...
    log.info(
        MODULE,
        `  customerSearch: ${customerSearch.docId ? customerSearch.queryName : 'NONE'}`
    );
    ...
    if (threadId && adminAssigner.docId) {
        strategies.push({
            ...
            fn: () =>
                queryViaAdminAssigner(
                    session,
                    pageId,
                    threadId,
                    adminAssigner.docId,
                    adminAssigner.queryName
                ),
        });
    }
    ...
    if (threadKey && session.cquickToken && commItemHeader.docId) {
        strategies.push({
            ...
            fn: () =>
                queryViaCommItemHeader(
                    session,
                    pageId,
                    threadKey,
                    commItemHeader.docId,
                    commItemHeader.queryName
                ),
        });
    }
    ...
    log.info(
        MODULE,
        `Trying ${strategies.length} strategies: ${strategies.map((s) => s.name).join(', ')}`
    );
    ...
    for (const [i, strategy] of strategies.entries()) {
        try {
            ...
            if (result) {
                log.info(
                    MODULE,
                    `[${i + 1}/${strategies.length}] ${strategy.name} → SUCCESS: ${result}`
                );
                return result;
            }
            ...
            log.info(
                MODULE,
                `[${i + 1}/${strategies.length}] ${strategy.name} → ERROR: ${err.message}`
            );
        }
    }
...
async function queryViaAdminAssigner(session, pageId, threadId, docId, queryName) {
    ...
    log.info(
        MODULE,
        `${queryName}: response ${text.length} bytes, first 500: ${text.substring(0, 500)}`
    );
    ...
    if (result?.errors?.length > 0) {
        log.info(
            MODULE,
            `${queryName}: GraphQL errors: ${JSON.stringify(result.errors.map((e) => e.message || e.description || e.summary)).substring(0, 300)}`
        );
    }
    ...
    if (result?.data) {
        ...
        for (const [key, val] of Object.entries(result.data)) {
            if (val && typeof val === 'object') {
                log.info(
                    MODULE,
                    `${queryName}: data.${key} keys: ${JSON.stringify(Object.keys(val)).substring(0, 200)}`
                );
            }
        }
    }
...
async function queryViaCommItemHeader(session, pageId, threadKey, docId, queryName) {
    ...
    if (result?.errors?.length > 0) {
        log.info(
            MODULE,
            `${queryName}: errors: ${JSON.stringify(result.errors.map((e) => e.message || e.summary)).substring(0, 300)}`
        );
    }
...
async function queryViaConversationPage(session, pageId, threadId) {
    ...
    if (threadIdIndex !== -1) {
        ...
        if (otherUserMatch && otherUserMatch[1] !== pageId) {
            log.info(
                MODULE,
                `ConversationPage: found other_user_id=${otherUserMatch[1]} near threadId`
            );
    ...
    if (allTargetIds.size > 0) {
        ...
        log.info(
            MODULE,
            `ConversationPage: ${allTargetIds.size} unique target_ids found, top: ${sorted
                .slice(0, 3)
                .map(([id, c]) => `${id}(×${c})`)
                .join(', ')}`
        );
    ...
    log.info(
        MODULE,
        `ConversationPage: no target_id extracted. HTML ${html.length} bytes, hasSelectedItem=${hasSelectedItem}, hasTargetId=${hasTargetId}, hasOtherUser=${hasOtherUser}, hasCommItem=${hasCommItem}`
    );
    return null;
}
...
async function queryViaInboxSearch(session, pageId, threadId) {
    ...
    log.debug(
        MODULE,
        `thread_info.php: HTTP ${response.status}, ${text.length} bytes, first 200: ${text.substring(0, 200)}`
    );
    ...
    if (result?.payload) {
        log.debug(
            MODULE,
            `thread_info.php: payload keys: ${Object.keys(result.payload).join(', ')}`
        );
    }
    if (result?.error) {
    ...
    if (threads && threads.length > 0) {
        ...
        const customer = participants.find(
            (p) => p.fbid !== pageId && String(p.fbid) !== String(pageId)
        );
        if (customer) {
...
async function findThread(session, pageId, docIds, opts) {
    ...
    const docId =
        docIds?.['MessengerThreadlistWebGraphQLQuery'] ||
    ...
    const nodes =
        result?.data?.viewer?.message_threads?.nodes ||
    ...
    for (const node of nodes) {
        ...
        if (_isAllActorArePage(node) && threadId) {
            const participantIds = (node.all_participants?.edges || []).map(
                (e) => e?.node?.messaging_actor?.id
            );
...
function _tryNextCategory(session, pageId, docIds, opts) {
    ...
    if (!isRetryWithNow) {
        log.debug(
            MODULE,
            'findThread: all categories exhausted, retrying from main with current time'
        );
        return findThread(session, pageId, docIds, {
            ...opts,
            category: 'main',
...
function _extractGlobalIdFromThread(thread, pageId) {
    ...
    if (thread.page_comm_item?.target_id) {
        log.info(
            MODULE,
            `findThread: found via page_comm_item.target_id=${thread.page_comm_item.target_id}`
        );
    ...
    if (!docId) {
        ...
        try {
            ...
            while ((match = regex.exec(html)) !== null) {
                const name = match[2];
                if (
                    name.includes('BizInboxCustomerRelay') ||
                    name.includes('CustomerSearch') ||
                    name.includes('UnifiedCustomerSearch')
                ) {
                    docId = match[1];
    ...
    for (const edge of edges) {
        ...
        if (threadId) {
            ...
            if (match?.node?.target_id) {
                log.info(
                    MODULE,
                    `getUserInboxByName: matched by threadId, target_id=${match.node.target_id}`
                );
        ...
        if (fbComms.length > 0 && fbComms[0]?.node?.target_id) {
            log.info(
                MODULE,
                `getUserInboxByName: first result target_id=${fbComms[0].node.target_id}`
            );
