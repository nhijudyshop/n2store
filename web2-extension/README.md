...
| ----------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| Trigger extension | Mọi lỗi Pancake (mode N2Store đi thẳng)                                                                         | Mọi lỗi Pancake _(trước: chỉ #10/2018278 + #551)_   |
| Lấy global_id     | `pdm.fetchMessages()` → `conversation.thread_id` + `page_customer.global_id` + `customers[].global_id` fallback | Cùng pattern _(trước: chỉ `buildConvData()` 1 lần)_ |
