                ...
                <div class="px-4 py-2 border-t border-border-light dark:border-border-dark">
                    ...
                    <div class="space-y-2">
                        ${activities.slice(0, 10).map(act => {
                            ...
                            // Build creator/operator label
                            let operatorHtml = '';
                            if (createdBy) {
                                if (isCancelled) {
                                    operatorHtml = ` · <span class="font-bold" style="color: #ef4444;">Người hủy: ${createdBy}</span>`;
                                } else if (isCreated) {
                                    operatorHtml = ` · <span class="font-bold text-blue-500">Người tạo: ${createdBy}</span>`;
                                } else if (isDeposit) {
                                    operatorHtml = ` · <span class="font-bold" style="color: #ef4444;">Duyệt bởi: ${createdBy}</span>`;
                                } else if (isRefund) {
                                    operatorHtml = ` · <span class="font-bold text-blue-500">${createdBy}</span>`;
                                } else {
                                    operatorHtml = ` · <span class="font-medium text-slate-500">${createdBy}</span>`;
                                }
                            }
