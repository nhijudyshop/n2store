...
// =====================================================
// AUTO-SYNC: Tự động thêm trang mới từ MENU_CONFIG
// Khi navigation-modern.js thêm trang mới, permissions
// registry tự động cập nhật mà không cần sửa file này
// =====================================================
function syncFromMenuConfig() {
    ...
    menuConfig.forEach(item => {
        ...
        if (PAGES_REGISTRY[pageId]) return; // Already exists

        // Auto-generate registry entry from MENU_CONFIG
        PAGES_REGISTRY[pageId] = {
            id: pageId,
            name: item.text || pageId,
            shortName: item.shortText || item.text || pageId,
            icon: item.icon || 'file',
            href: item.href || '',
            description: item.text || pageId,
            adminOnly: item.adminOnly || false,
            publicAccess: item.publicAccess || false,
...
// navigation-modern.js loads with defer, so MENU_CONFIG won't be ready yet
// Run sync after DOM is ready (when deferred scripts have executed)
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => syncFromMenuConfig());
} else {
    // DOM already loaded, try now and fallback with delay
    if (!syncFromMenuConfig()) {
        setTimeout(syncFromMenuConfig, 500);
    }
}
