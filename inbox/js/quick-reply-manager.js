...
class QuickReplyManager {
    ...
    async sendQuickReplyWithImage(imageUrl, message) {
        ...
        try {
            ...
            const pageAccessToken = await window.inboxTokenManager?.getOrGeneratePageAccessToken(channelId);
            if (!pageAccessToken) {
            ...
            const apiUrl = InboxApiConfig.buildUrl.pancakeOfficial(
            ...
            const sendImage = async () => {
                ...
                const pancakeApiUrl = InboxApiConfig.buildUrl.pancakeOfficial(
                    `pages/${channelId}/conversations/${conversationId}/messages`,
                    pageAccessToken
                ) + (customerId ? `&customer_id=${customerId}` : '');
                ...
                const imageResponse = await InboxApiConfig.smartFetch(pancakeApiUrl, {
                    method: 'POST',
                    headers: {
            ...
            const sendText = async () => {
                ...
                // Call Pancake API via proxy (avoid CORS)
                const textResponse = await InboxApiConfig.smartFetch(pancakeApiUrl, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json'
    ...
    async uploadTemplateImage(blob) {
        ...
        try {
            ...
            const result = await window.inboxPancakeAPI.uploadImage(channelId, blob);
