...
console.log('🔨 N2Store Build Script - Core Bundle');
...
try {
    ...
    try {
        ...
        minify(bundleContent, {
            ...
            if (result.code) {
                ...
                console.log(`  ✅ Minified: ${(minSize / 1024).toFixed(2)} KB (-${saved}%)`);
                console.log(`  Output: ${path.relative(ROOT_DIR, minPath)}`);
            }
        }).catch(err => {
            console.warn('  ⚠️ Minification skipped:', err.message);
        });
    ...
    console.error('❌ Failed to write bundle:', error.message);
    process.exit(1);
}
