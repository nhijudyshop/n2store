...
export async function handleFacebookSend(request, url) {
    ...
    try {
        ...
        if (commentId && message && pageToken) {
            console.log(
                '[FACEBOOK-SEND] Direct Private Reply mode → commentId:',
                commentId,
                'pageId:',
                pageId
            );
            ...
            if (pageId) {
                ...
                try {
                    ...
                    console.log(
                        '[FACEBOOK-SEND] Direct Private Reply response:',
            ...
            console.warn(
                '[FACEBOOK-SEND] No pageId for Direct Private Reply, skipping to normal flow'
            );
        }
        ...
        async function sendWithTagFallback(baseFbBody) {
            if (!useTag) {
                ...
                console.log(
                    '[FACEBOOK-SEND] Sending without tag, body:',
                    JSON.stringify(baseFbBody)
                );
            ...
            for (const tag of TAG_SEQUENCE) {
                ...
                console.warn(
                    `[FACEBOOK-SEND] ⚠️ ${tag} failed (${result.error.code}): ${result.error.message}`
                );
            }
        ...
        async function searchCommentsOnObject(
            objectId,
            customerAsid,
            customerName,
            filter,
            ...
            try {
                ...
                const uniqueCommenters = [
                    ...new Map(
                        comments
                            .filter((c) => c.from)
                            .map((c) => [c.from.id, { id: c.from.id, name: c.from.name }])
                    ).values(),
                ];
                ...
                const idMatches = comments.filter(
                    (c) => c.from && String(c.from.id) === String(customerAsid)
                );
                ...
                if (customerName) {
                    ...
                    const nameMatches = comments.filter(
                        (c) =>
                ...
                if (matches.length > 0) {
                    console.log(
                        `[PRIVATE-REPLY] Found ${matches.length} matching comments (excl ${excludeIds.size} already tried)`
                    );
                }
        ...
        async function findAllMatchingComments(
            fbPostId,
            customerAsid,
            customerName,
            excludeIds = new Set()
            ...
            if (fbPostId) {
                ...
                for (const pid of postIdVariants) {
                    for (const filter of ['stream', null]) {
                        const results = await searchCommentsOnObject(
                            pid,
                            customerAsid,
                            customerName,
                            filter,
            ...
            if (allFailed || !fbPostId) {
                ...
                try {
                    ...
                    if (!lvData.error && lvData.data?.length > 0) {
                        console.log(
                            `[PRIVATE-REPLY] Found ${lvData.data.length} live videos on page`
                        );
                        diagnostics.liveVideos = lvData.data.map((v) => ({
                            id: v.id,
                            title: v.title,
                        ...
                        for (const video of lvData.data) {
                            console.log(
                                `[PRIVATE-REPLY] Searching live video: ${video.id} (${video.title || 'untitled'})`
                            );
                            const results = await searchCommentsOnObject(
                                video.id,
                                customerAsid,
                        ...
                        diagnostics.queries.push({
                            query: `${pageId}/live_videos`,
                            error: lvData.error?.message || 'no data',
                        });
                    }
                } catch (err) {
                    diagnostics.queries.push({
                ...
                try {
                    ...
                    if (!feedData.error && feedData.data?.length > 0) {
                        console.log(
                            `[PRIVATE-REPLY] Found ${feedData.data.length} posts in page feed`
                        );
                        diagnostics.feedPosts = feedData.data.map((p) => ({
                            id: p.id,
                            type: p.type,
                        ...
                        for (const post of feedData.data) {
                            console.log(
                                `[PRIVATE-REPLY] Searching post: ${post.id} (${post.type || 'unknown'})`
                            );
                            const results = await searchCommentsOnObject(
                                post.id,
                                customerAsid,
                        ...
                        diagnostics.queries.push({
                            query: `${pageId}/feed`,
                            error: feedData.error?.message || 'no data',
                        });
                    }
                } catch (err) {
            ...
            console.log(
                `[PRIVATE-REPLY] Total unique matches: ${uniqueMatches.length} (after dedup & excluding ${excludeIds.size})`
            );
        ...
        async function sendPrivateReply(realCommentId, messageText) {
            const sendUrl = `${API_ENDPOINTS.FACEBOOK.GRAPH_URL}/${pageId}/messages`;
            console.log(
                '[PRIVATE-REPLY] Sending Private Reply via Send API, comment:',
                realCommentId,
                'page:',
                pageId
            );
            ...
            console.log(
                '[PRIVATE-REPLY] ✅ Private Reply sent successfully!',
                JSON.stringify(result)
            );
        ...
        if (sendApiFailed && hasTextMessage) {
            console.log('[FACEBOOK-SEND] ========================================');
            console.log(
                `[FACEBOOK-SEND] Send API failed (${lastError?.code}) → trying Private Reply fallback`
            );
            ...
            if (knownCommentIds && Array.isArray(knownCommentIds) && knownCommentIds.length > 0) {
                console.log(
                    `[FACEBOOK-SEND] Trying ${knownCommentIds.length} known comment IDs from order data`
                );
                ...
                for (const knownId of knownCommentIds) {
                    ...
                    if (prResult.success) {
                        console.log(
                            '[FACEBOOK-SEND] ✅ Private Reply succeeded with known commentId!'
                        );
                    ...
                    console.warn(
                        `[FACEBOOK-SEND] Known commentId ${knownId} failed (${errCode}):`,
                        prResult.result?.error?.message
                    );
                }
                console.log(
                    '[FACEBOOK-SEND] All known comment IDs failed, searching for more comments...'
                );
            }

            // === STEP B: Search ALL matching comments on page posts (excluding already tried) ===
            const { matches, diagnostics } = await findAllMatchingComments(
                postId,
                psid,
                customerName,
                triedCommentIds
            ...
            if (matches.length > 0) {
                console.log(
                    `[FACEBOOK-SEND] Found ${matches.length} new comments to try via Private Reply`
                );

                for (const match of matches) {
                    console.log(
                        `[FACEBOOK-SEND] Trying commentId: ${match.commentId} (matched by: ${match.matchedBy})`
                    );
                    ...
                    const errCode = prResult.result?.error?.code;
                    console.warn(
                        `[FACEBOOK-SEND] Comment ${match.commentId} failed (${errCode}):`,
                        prResult.result?.error?.message
                    );
                    if (errCode === 10900) allRepliedTo = true;
                }
                ...
                return jsonResponse(
                    {
                        success: false,
                        error:
                            lastError?.message || 'Không tìm thấy comment của khách trên bài viết',
 ...
 */
// GATE (2026-06-21): /api/facebook-graph là proxy Graph GET MỞ — bất kỳ ai có
// access_token đều đọc được node Graph tuỳ ý (open relay). Hiện KHÔNG còn caller
// frontend nào dùng route worker này: web2-fb-posts đi qua Render server riêng;
// orders-report dùng TPOS `/api/facebook-graph/*` (khác host); `/livevideo` là route
// FACEBOOK_LIVE riêng. → siết allowlist read-only an toàn, KHÔNG ảnh hưởng Web 1.0/2.0.
// Host upstream cố định là graph.facebook.com (GRAPH_URL) nên `path` không thể đổi
// host; allowlist chỉ giới hạn edge đọc + chặn traversal/scheme injection.
const FB_GRAPH_ALLOW = [
    /^me$/,
    /^me\/(accounts|adaccounts|businesses)$/,
    /^debug_token$/,
    /^oauth\/access_token$/,
    /^\d+$/, // node by id
    /^\d+\/(comments|feed|posts|published_posts|live_videos|videos|insights|attachments|reactions|likes|conversations|tagged|picture)$/,
    /^act_\d+(\/(insights|campaigns|adsets|ads|adcreatives))?$/,
];

/**
 * Chuẩn hoá + kiểm tra Graph path theo allowlist read-only.
 * @param {string} rawPath - giá trị `?path=`
 * @returns {string|null} path đã chuẩn hoá nếu hợp lệ, null nếu bị chặn.
 */
function normalizeAllowedGraphPath(rawPath) {
    if (typeof rawPath !== 'string') return null;
    const p = rawPath.trim().replace(/^\/+/, '').replace(/\/+$/, '');
    if (!p || p.length > 256) return null;
    if (/[:\\@]/.test(p)) return null; // scheme (://), backslash, userinfo @
    if (p.includes('..')) return null; // path traversal
    if (p.includes('//')) return null; // double slash / host injection
    return FB_GRAPH_ALLOW.some((re) => re.test(p)) ? p : null;
}

export async function handleFacebookGraph(request, url) {
    const path = url.searchParams.get('path');
    const accessToken =
        url.searchParams.get('access_token') || request.headers.get('X-Page-Access-Token');

    if (!path || !accessToken) {
        return errorResponse('Missing path or access_token', 400);
    }

    // GET-only (OPTIONS preflight đã short-circuit ở worker.js trước dispatch).
    if (request.method !== 'GET') {
        return errorResponse('facebook-graph chỉ hỗ trợ GET', 405);
    }

    // Allowlist read-only — chặn open-relay đọc node Graph tuỳ ý.
    const safePath = normalizeAllowedGraphPath(path);
    if (!safePath) {
        console.warn(`[FB-GRAPH] BLOCKED path (không thuộc allowlist): ${path}`);
        return errorResponse('Graph path không được phép', 403);
    }
    ...
    const fbUrl = `${API_ENDPOINTS.FACEBOOK.GRAPH_URL}/${safePath}?${fbParams.toString()}`;
    console.log(`[FB-GRAPH] Proxying: ${safePath}`);
...
export async function handleFacebookLiveVideos(request, url) {
    ...
    try {
        ...
        newResponse.headers.set(
            ...
            'Content-Type, Authorization, Accept, tposappversion, x-tpos-lang'
        );
