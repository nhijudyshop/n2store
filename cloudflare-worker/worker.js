...
import {
    ...
    errorResponse,
    addCorsHeaders,
} from './modules/utils/cors-utils.js';
...
import {
    ...
    handlePancakeAvatar,
    handleImgbbUpload,
} from './modules/handlers/image-proxy-handler.js';
import {
    handleFacebookSend,
    handleFacebookLiveVideos,
    handleFacebookGraph,
} from './modules/handlers/facebook-handler.js';
import {
    handlePancakeDirect,
    handlePancakeOfficial,
    handlePancakeOfficialV2,
    handlePancakeGeneric,
    handlePancakeWebSocket,
} from './modules/handlers/pancake-handler.js';
import {
...
async function routeRequest(request, env, ctx) {
    ...
    try {
        ...
        switch (route) {
            ...
            case 'WEB2_USERS':
                return handleCustomer360Proxy(request, url, pathname);
            case 'WEB2_QUICK_REPLIES':
                return handleCustomer360Proxy(request, url, pathname);
            case 'WEB2_SUPPLIER_WALLET':
                return handleCustomer360Proxy(request, url, pathname);
            case 'WEB2_SO_ORDER':
                return handleCustomer360Proxy(request, url, pathname);
            case 'WEB2_MSG_TEMPLATES':
                return handleCustomer360Proxy(request, url, pathname);
