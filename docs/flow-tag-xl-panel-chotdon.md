...
| 8 | Khách boom | `KHACH_BOOM` | Manual | 💥 | Khách boom/hủy đơn |
| 9 | Thẻ khách lạ | `THE_KHACH_LA` | Manual | 🪪 | Thẻ khách lạ |
| 10 | Đã đi đơn gấp | `DA_DI_DON_GAP` | Manual | ⚡ | Đơn đã đi gấp |
| 11 | Khác | `KHAC` | Manual | 📋 | Ghi chú tự do |
...
| 13 | Auto TAG XL → TPOS Sync | **ACTIVE** | Gán/bỏ TAG XL flag/subtag/ttag | Gán/xóa TPOS tag tương ứng |
...
| 2 | **Fallback (mọi tag không match mapping)** | Add flag `KHAC` | ❌ Add-only |

### Fallback Rule — Bất Kỳ TPOS Tag Lạ → Flag KHAC

> Bất kỳ TPOS tag nào **không match** với static mapping, alias, T-number pattern, hoặc custom flag label → tự động add flag `KHAC` (add-only, không auto-remove).

**TPOS tag được coi là "known" (skip fallback)** khi:
- Có trong `PTAG_TO_TPOS_MAP` (kể cả subtags GIỎ TRỐNG / ĐÃ GỘP KO CHỐT / NCC HẾT HÀNG — vì chúng là "known but skipped" cho category)
- Có trong `TPOS_ALIAS_MAP` (TRỪ THU VỀ, KHÁCH CK)
- Match `T\d+ [mô tả]` pattern (handled riêng bằng T-tag auto-create)
- Trùng `label` của 1 custom flag

**Tag rơi vào fallback (→ KHAC)** bao gồm (không giới hạn):

| Pattern | Ví dụ | → |
|---------|-------|---|
| `OK [seller]` | OK DUYÊN, OK HẠNH, OK BO | flag KHAC |
| `XỬ LÝ [seller]` | XỬ LÝ HẠNH | flag KHAC |
| `XÃ ĐƠN [seller]` | XÃ ĐƠN BO | flag KHAC |
| `Gộp xxx yyy` | Gộp ABC DEF | flag KHAC |
| `K\d+ xxx` | K8 CHECK IB | flag KHAC |
| `CỌC xxxK` | CỌC 100K, CỌC 200K | flag KHAC |
| `BÁN HÀNG NHA`, `XÃ KHÁCH LẠ`, etc. | (bất kỳ tag lạ) | flag KHAC |

### Không Sync (skip TPOS → XL)
...
Sync được hook vào 4 hàm TAG XL (fire-and-forget, không block UI):

| Hàm | Khi nào | Action |
|-----|---------|--------|
| `assignOrderCategory()` | Gán category có subTag trong mapping | `add` TPOS tag + `remove` subTag cũ |
| `toggleOrderFlag()` | Toggle flag on/off | `add` hoặc `remove` TPOS tag |
| `assignTTagToOrder()` | Gắn T-tag | `add` TPOS tag |
| `removeTTagFromOrder()` | Tháo T-tag | `remove` TPOS tag |

### Flow & Fallback Chain

```
Seller gán TAG XL (VD: flag KHÁCH BOOM)
  │
  ├── 1. TAG XL lưu bình thường (API + SSE)
  │
  └── 2. Fire-and-forget: syncPtagToTPOS(orderCode, 'add', 'flag:KHACH_BOOM')
          │
          ├── Resolve TPOS tag name: "KHÁCH BOOM" (từ PTAG_TO_TPOS_MAP)
          │
          ├── Tìm trong window.availableTags (cache local)
          │   ├── ✅ Found → tiếp tục
          │   └── ❌ Not found ↓
          │
          ├── FALLBACK 1: Reload tất cả tags từ API
          │   GET /api/odata/Tag?$top=1000&$count=true
          │   ├── ✅ Found → tiếp tục
          │   └── ❌ Not found ↓
          │
          ├── FALLBACK 2: Tạo TPOS tag mới
          │   POST /api/odata/Tag { Name: "KHÁCH BOOM", Color: random }
          │   └── ✅ Created → tiếp tục
          │
          ├── Lấy TPOS tags hiện tại của đơn (từ OrderStore)
          │   JSON.parse(order.Tags)
          │
          ├── Add/Remove TPOS tag khỏi danh sách
          │
          └── Gọi AssignTag API
              POST .../ODataService.AssignTag
              { Tags: [...updatedList], OrderId }
              → Update OrderStore + updateRowTagsOnly + emitTagUpdateToFirebase
```

### Sync 2 chiều (Add + Remove)

- **Gán** TAG XL flag/subtag/ttag → **thêm** TPOS tag tương ứng vào đơn
- **Bỏ** TAG XL flag/subtag/ttag → **xóa** TPOS tag tương ứng khỏi đơn
- **Chuyển** category (VD: Cat 2/CHUA_PHAN_HOI → Cat 3/GIO_TRONG) → xóa tag cũ + thêm tag mới

### Custom Flags → TPOS

Custom flag (key `CUSTOM_xxx`) dùng **label** (uppercase) làm tên TPOS tag.

VD: Custom flag label "GIẢM GIÁ 50%" → TPOS tag name "GIẢM GIÁ 50%"

### Lưu ý

- Sync chạy **fire-and-forget** — lỗi chỉ log console, không block TAG XL flow
- Nếu TPOS API down → TAG XL vẫn hoạt động bình thường, chỉ mất sync TPOS
...
| `THẺ KHÁCH LẠ` | `flag:THE_KHACH_LA` | PTAG_TO_TPOS_MAP | ✅ Add/Remove |
| `ĐÃ ĐI ĐƠN GẤP` | `flag:DA_DI_DON_GAP` | PTAG_TO_TPOS_MAP | ✅ Add/Remove |
| `GIẢM GIÁ` | `flag:GIAM_GIA` | PTAG_TO_TPOS_MAP | ✅ Add/Remove |
| `CHUYỂN KHOẢN` | `flag:CHUYEN_KHOAN` | PTAG_TO_TPOS_MAP | ✅ Add/Remove |
| `KHÁCH CK` | `flag:CHUYEN_KHOAN` | TPOS_ALIAS_MAP | ✅ Add/Remove |
| `QUA LẤY` | `flag:QUA_LAY` | PTAG_TO_TPOS_MAP | ✅ Add/Remove |
| `CHỜ LIVE` | `flag:CHO_LIVE` | PTAG_TO_TPOS_MAP | ✅ Add/Remove |
| `GIỮ ĐƠN` | `flag:GIU_DON` | PTAG_TO_TPOS_MAP | ✅ Add/Remove |
| `MY THÊM CHỜ VỀ` | `ttag:T_MY` | PTAG_TO_TPOS_MAP | ✅ Add/Remove |
| `T\d+ [mô tả]` (pattern) | `ttag:T*` (auto find/create) | Pattern detect | ✅ Add/Remove |
| **(mọi tag lạ khác)** | `flag:KHAC` (add-only) | Fallback | ⚠ Add-only |
| Custom flag labels | Custom flag IDs | Reverse lookup | ✅ Add/Remove |
...
| `orders-report/js/tab1/tab1-tag-sync.js` | ~930 | TPOS ↔ Tag XL sync: modal + logic + reverse auto sync |
