...
> Phân tích toàn bộ source code website Pancake.vn từ HTML, JS bundles, **NEXT_DATA**, và build manifest.
...
| 5   | [WebSocket Events](#5-websocket-events-phoenix-channels)  | 50+ Phoenix channel events: pages:_, users:_, content:\*, calling, WhatsApp                     | ~535  |
...
```json
{
    "psid": "string",
    "message_id": "string",
    "watermark": "number (unix ts)",
    "is_group_conv": "boolean|null"
}
```
...
| ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /api/v1/pages/{pageId}/avatar?access_token={JWT}`                         | `200 image/jpeg ~5-6 KB`                                                                                                                                                                                                                               | —                                                                          | **Page avatar — works for cả FB và IG**. `/api/v1/pages` list chỉ fill `avatar_url` cho IG (`cdninstagram.com`); FB pages trả `avatar_url: null`. Endpoint /avatar không cần fbId — dùng cho page selector display.                                 |
...
```json
{
  "id": "270136663390370_25717004554573583",
  "type": "INBOX",
  "from_psid": "25717004554573583",
  "customers": [{"fb_id": "...", "id": "<UUID>", "name": "..."}],
  "recent_phone_numbers": [
    {"captured": "0908123456", "length": 10, "m_id": "m_xxx", "offset": 0, "phone_number": "0908123456", "status": 2}
  ],
  "thread_id": "32546288751686299",
  "last_sent_by": {
    "admin_name": "Public API",
    "id": "117267091364524",    // = pageId khi shop gửi cuối (KEY field!)
    "name": "Nhi Judy House",
    "uid": "0c7cf221-d058-..."  // Pancake user UUID
  },
  "last_message": null | {from: {id, name}, message, inserted_at},
  "last_customer_interactive_at": "2026-04-22T02:07:08",
  "updated_at": "2026-05-10T04:17:24.407000",
  "unread_count": 0,
  "message_count": 41,
  "snippet": "...",
  "seen": true,
...
| Global                              | Type                   | Purpose                                                                      |
| ----------------------------------- | ---------------------- | ---------------------------------------------------------------------------- |
| `window.__pancakeReduxStore__`      | Redux store            | Full state — `.getState()` returns 17 slices (xem section 3)                 |
| `window.__pancakeReduxHistoryLog__` | Array                  | Action history log (rất ngắn, chỉ 1-2 entries)                               |
| `window.__NEXT_DATA__`              | Object (in script tag) | Next.js initial server-rendered state, ~327 KB JSON với toàn bộ initialState |
...
**System (3)**: `phx_close`, `phx_error`, `phx_reply`

**WhatsApp integration (12)**: `users:wa_qrcode`, `users:wa_pairing_code`, `users:wa_noti`, `users:wa_disconnected`, `users:wa_activated`, `users:wa_page_avatar_tag`, `pages:wa_update_name`, `pages:wa_qrcode`, `pages:wa_pairing_code`, `pages:wa_page_avatar_tag`, `pages:wa_contacts`, `pages:wa_mobile_has_connection`, `pages:wa_start_check_has_mobile_connection`

**TikTok integration (5+)**: `pages:tiktok_connected`, `pages:tiktok_qrcode_scanned`, `pages:tiktok_seller_required_2fa`, `pages:tiktok_seller_connected`, `pages:tiktok_seller_error`, ...

**Instagram (1+)**: `pages:ig:connected`, ...

**User account (8+)**: `users:tl_login_session`, `users:pzl_connected`, `users:pzl_new_qr`, `users:deactive_current_session`, `users:active_subscription_success`, `users:show_notifications`, `users:hide_notifications`, `users:update_notifications`, `users:add_page`, `users:new_pages`

**Conversation core (3)**: `pages:new_message`, `pages:update_conversation` (carries `unread_count`, `last_sent_by`, `snippet`, `seen`), `pages:update_conversation_avatar`

**Routing/config**: `active_config_routing_page_settings`
...
| Path tried                                            | Status | Note                                                          |
| ----------------------------------------------------- | ------ | ------------------------------------------------------------- |
| `PATCH /api/v1/pages/{pid}/customers/{uuid}`          | 404    | Update customer fields                                        |
| `POST .../customers/{uuid}/phone_numbers`             | 404    | Add phone                                                     |
| `POST .../customers/{uuid}/recent_phone_numbers`      | 404    | Update recent phones                                          |
| `POST .../customers/{uuid}/edit_phone`                | 404    | Edit phone                                                    |
| `POST .../customers/{uuid}/add_phone_number`          | 404    | Add phone                                                     |
| `POST /api/v1/customers/update_global_id`             | 404    | Merge customer                                                |
| `GET /api/v1/customers_by_phone_number?phone_number=` | 500    | Broken/protected                                              |
| `GET /api/v1/customers/search?q=`                     | 406    | Wrong content-type or not exposed                             |
| `GET /api/v1/customers?phone=`                        | 406    | Wrong content-type or not exposed                             |
| `GET /api/v1/customers/by-phone/{phone}`              | 406    | Not exposed                                                   |
| `GET /api/v1/pages/{pid}/customers?phone=`            | 406    | Not exposed                                                   |
| `GET /api/v1/conversations/search?q=`                 | 500    | Page-scoped version `/pages/{pid}/conversations/search` works |
...
└── DIV.app-navbar__container
    └── DIV.app-navbar__wrapper
        ├── UL#nav-bar-main-menu.app-navbar.app-navbar__left  (5 main tabs)
        │   ├── LI.nav-item-link.active        → "Hội thoại"      /{pageSlug}
        │   ├── LI.nav-item-link               → "Đơn hàng"       (external — pos.pages.fm)
        │   ├── LI.nav-item-link               → "Bài viết"       /{pageSlug}/post
        │   ├── LI.nav-item-link               → "Thống kê"       /{pageSlug}/statistic/overview
        │   └── LI.nav-item-link               → "Cài đặt"        /{pageSlug}/setting/general
        └── .app-navbar__right
            ├── LI.sip-nav-item                (VoIP/SIP indicator)
            ├── LI.avatar-navbar.custom-dropdown-menu  (page selector dropdown)
            └── LI.custom-dropdown-menu        (user menu)
```
...
| -------------- | ---------------------- | --------- | ----------------------- |
| Nhi Judy       | `igo_nhijudyoi`        | Instagram | `igo_17841474211813256` |
| NhiJudy Nè     | `NhiJudy.Boutique`     | Facebook  | `193642490509664`       |
| Sách Nói       | `sachnoi.vietnam`      | Facebook  | `324873917372219`       |
| Nhi Judy House | `NhiJudyHouse.VietNam` | Facebook  | `117267091364524`       |
| NhiJudy Store  | `NhiJudyStore`         | Facebook  | `270136663390370`       |
| Nhi Judy Ơi    | `NhiJudy.Style`        | Facebook  | `112678138086607`       |
...
- `q=0123456788` trên Store trả 1 conv "Huỳnh Thành Đạt" với phone `0908123456` (homonym) — match vì message body của conv đó từng chứa "0123456788" làm tham chiếu.
