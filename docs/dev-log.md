...
function someOp(...args) {
    const prev = snapshot();
    applyOptimistic();         // STATE + DOM update sync
    showToast('✓ Done');       // optimistic toast NGAY
    (async () => {
        try {
            const r = await fetch(...);
            const d = await r.json();
            if (!d.success) throw new Error(d.error);
            syncAuthoritative(d); // silent re-sync nếu cần
        } catch (e) {
            rollback(prev);        // restore STATE + DOM
...
- Backend background fail (mock 500 sau 200ms): T+500ms badge rolled back (display:none), undo toast removed, error notif fire qua notificationManager: `["✗ Lỗi thêm \"BAD-X\": simulated 500", "error"]`.
