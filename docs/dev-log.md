...
### [wallet-display] Ghi rõ nguyên nhân điều chỉnh ví: "chuyển từ SĐT X → SĐT Y" ✅
| | |
|---|---|
| **Files** | `customer-hub/js/modules/customer-profile.js`, `customer-hub/js/modules/wallet-panel.js`, `orders-report/js/tab1/tab1-wallet-modal.js` |
| **Chi tiết** | Bổ sung dòng mô tả tường minh cho mỗi giao dịch ADJUSTMENT: `Điều chỉnh ví sai SĐT: chuyển số dư từ SĐT <wrong> → SĐT <correct> (+/- X tiền)`, kèm dòng `Lý do: ...`. Áp ở cả 3 nơi render: customer-profile activity feed, wallet-panel `_renderManualTxItem` + `_renderTx`, tab1-wallet-modal. Dùng `wrong_customer_phone`/`correct_customer_phone` từ JOIN backend (đã trả từ commit `6d68252b`). Khi không có `correct_customer_phone` (debit_only) thì hiển thị fallback ngắn hơn. |
| **Status** | ✅ Done |
