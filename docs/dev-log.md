...
- **Modal trả cả đơn = FULL browser** (user: "modal full browser chứ không phải ảnh"): `.pr-bulk-content` width/height 96vw×94vh (đo được ~92% viewport), flex column → `#prBulkForm` flex:1, `.pr-bulk-table-wrap` flex:1 (cuộn trong) → bảng SP chiếm phần lớn chiều cao, form (Lý do/Phương thức/Ghi chú/Tổng/nút) neo đáy luôn trong tầm nhìn. Bump v20260617e.
