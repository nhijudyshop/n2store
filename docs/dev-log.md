...
**Diagnose** (browser test localhost):

- `Report/DeliveryReport` (endpoint cũ): 566 đơn cho range 09–10/05 (50 + 516).
- `FastSaleOrder/ODataService.GetView` cùng range: **778 đơn** (50+45 open + 5+28 paid + 1+89 cancel + 1+121 draft).
- → TPOS `Report/DeliveryReport` âm thầm lọc `State ∈ {draft, cancel}`, làm thiếu đúng các đơn đã hủy / chưa lên đơn.
- 2 đơn user thấy thiếu (NJD/2026/65944, NJD/2026/66138) đều `State=cancel` — đã lock vào DB ngày 09/05 nhưng sau đó bị hủy trong TPOS, endpoint cũ không trả về nữa.

**Fix** ([delivery-report.js](../delivery-report/js/delivery-report.js)):

- `buildApiUrl()` — đổi sang `FastSaleOrder/ODataService.GetView` với `$filter=Type eq 'invoice' and (State eq 'open' or State eq 'paid' or State eq 'cancel') and DateInvoice ge ... and DateInvoice le ...`. Loại draft (chưa lên đơn, không phải invoice thật) nhưng GIỮ cancel để user thấy đơn đã hủy. GetView không honor `ne` operator nên dùng OR explicit. Keyword search (non-phone) chuyển từ `Q` query param sang OData `contains(Number,'X') or contains(TrackingRef,'X')`.
- `renderStats()` — ưu tiên `State='cancel'` vào bucket "Trả hàng" (vì invoice hủy thì ShipStatus vẫn 'none'). COD tổng và "Đang giao" chỉ tính đơn chưa hủy. Fix sẵn bug cũ: `drStatShippingCount` từng dùng `totalCount` thay `shippingCount`.
- `renderTable()` — class `dr-row-cancelled` + badge "Đã hủy" trước Number. Tfoot money totals bỏ qua đơn cancel.
- `ensureProvinceGroups()` — banner "đã khóa" giờ chỉ đếm assignment có trong `allData` hiện tại (tránh ghost-count khi DB còn assignment của đơn đã out-of-range).

**CSS** ([delivery-report.css](../delivery-report/css/delivery-report.css)):

- `.dr-row-cancelled` — bg `#fef2f2`, text `#991b1b`, line-through cho customer/date/money cells.
- `.dr-cancel-badge` — pill đỏ "Đã hủy" inline trước order Number.

**Verify** (browser test localhost 09–10/05/2026):

- Trước fix: 566 dòng, banner 568 đã khóa, không thấy đơn hủy.
- Sau fix: **656 dòng** (533 open + 33 paid + 90 cancel), banner 656 đã khóa khớp số dòng, 90 dòng đỏ "Đã hủy", stats — Giao hàng thu tiền 566 hóa đơn / 332.211.000đ (active), Trả hàng 90 hóa đơn / 67.773.000đ (cancel + returned), Đang giao 566 / 332.211.000đ. Tfoot Tổng: 656 / 332.211.000đ (active money). Keyword search single order + phone search vẫn OK.

**Files**: MODIFIED [delivery-report/js/delivery-report.js](../delivery-report/js/delivery-report.js), MODIFIED [delivery-report/css/delivery-report.css](../delivery-report/css/delivery-report.css).
