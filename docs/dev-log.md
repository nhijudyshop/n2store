...
- Files: native-orders.js, web2-order-tags-service.js, web2-order-tag-detail.js (?v=kpi2), native-orders-render.js (?v=kpi), +native-orders-kpi-health.js (?v=kpi). Backend=Render deploy; FE=GH Pages. Test live: chốt base thật qua nút admin rồi verify amber→xanh + KPI qty.
