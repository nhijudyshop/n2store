...
> **Auto-generated** • 2026-07-01 03:47 • 465 files, 152 shared modules, 3902 hàm, 18 file > 800 dòng.
...
| `WEB2_CONFIG`, `API_CONFIG`, `Web2Auth`                 | [web2-auth.js](../../web2/shared/web2-auth.js)                                        | Token storage + verify + page guard.                                                                    | 149       |
...
| `Web2Escape`                                            | [web2-escape.js](../../web2/shared/web2-escape.js)                                    | WEB2.0 module.                                                                                          | 89        |
...
| `Web2Optimistic`                                        | [web2-optimistic.js](../../web2/shared/web2-optimistic.js)                            | Codifies pattern: snapshot → apply optimistic UI → fire backend background →                            | 29        |
...
#### `Web2Campaign` — [web2/shared/web2-campaign.js](../../web2/shared/web2-campaign.js) · 232 dòng
...
#### `Web2LiveTvDisplay` — [web2/shared/web2-live-tv-display.js](../../web2/shared/web2-live-tv-display.js) · 158 dòng
...
#### `Web2Sidebar` — [web2/shared/web2-sidebar.js](../../web2/shared/web2-sidebar.js) · 983 dòng
...
#### `Web2Tryon` — [web2/shared/web2-tryon.js](../../web2/shared/web2-tryon.js) · 792 dòng
...
- **[live-campaign-manager.js](../../live-chat/js/live/live-campaign-manager.js)** ·470 — WEB2.0 — quản lý chiến dịch cha (gom bài livestream) ngay trong live-chat.
    - exposes: `LiveCampaignManager`
    - uses shared: `Popup`, `API_CONFIG`, `Web2Auth`, `Web2Campaign`, `Web2Optimistic`
    - funcs (21): API, \_api, \_close, \_injectStyles, \_mount, \_open, \_pagePosts, \_removeBackBanner, \_render, \_showBackBanner, \_viewCampaign, \_w2AuthHeaders, apply, campOpts, doRun, esc, exitCampaignView, onSuccess, rollback, run, snapshot
...
- **[native-orders-filters-campaigns.js](../../native-orders/js/native-orders-filters-campaigns.js)** ·459 — WEB2.0 module.
    - exposes: `NativeOrders`
    - uses shared: `Web2Campaign`
    - funcs (27): \_searchSuggestItems, \_syncChannelUi, \_tagSummary, \_visibleOrders, applyFilters, applyTagFilter, clearFilters, clearParentSelection, clearTagFilter, firstMatch, hideSearchSuggest, loadAvailableCampaigns, loadCampaignSelection, loadParentCampaigns, moveSuggestActive, pickNewestHouseStore, pickSuggestion, reconcileCampaignSelection, renderCampaignDropdown, renderCampaignLabel, renderParentCampaigns, renderSearchSuggest, row, saveCampaignSelection, selectParentCampaign, toggleCampaignDropdown, toggleFilter
...
- **[native-orders-realtime-init.js](../../native-orders/js/native-orders-realtime-init.js)** ·327 — WEB2.0 module.
    - exposes: `NativeOrders`
...
- **[native-orders-render.js](../../native-orders/js/native-orders-render.js)** ⚠️1048 — WEB2.0 module.
    - exposes: `NativeOrders`
...
- **[cham-cong-app.js](../../web2/cham-cong/js/cham-cong-app.js)** ⚠️903 — WEB2.0 module — Chấm công: app core (state + bảng công + import + sync).
    - exposes: `ChamCong`
...
- **[cham-cong-salary.js](../../web2/cham-cong/js/cham-cong-salary.js)** ·304 — WEB2.0 module — Chấm công: tính lương (pure).
...
- **[goods-weight.js](../../web2/goods-weight/js/goods-weight.js)** ·656 — WEB2.0 — Cân nặng hàng khi về kiện.
    - uses shared: `Popup`, `WEB2_CONFIG`, `API_CONFIG`, `Web2Auth`, `Web2Drawer`, `Web2ImageLightbox`, `Web2SSE`
    - funcs (48): $, \_auth, addDays, api, boot, buildMonths, clearMonthActive, clearPhoto, compress, currentMonth, dayLabel, dayPhotosHtml, del, deleteDay, ensureDrawer, esc, fillUserOptions, fmtInt, fmtKg, fmtTime, icons, isAdmin, last12Months, load, loadReport, money, monthRange, onClose, onDateChange, onPhoto, onerror, onload, openDayDrawer, reload, render, renderReport, save, selectMonth, setPreset, setupReport, setupSSE, showTab, tickClock, toast, todayHCM, token, username, wireDrawerPhotos
...
- **[kpi-assignments.js](../../web2/kpi/js/kpi-assignments.js)** ·437 — WEB2.0 module.
...
- **[live-control.js](../../web2/live-control/js/live-control.js)** ⚠️1082 — WEB2.0 — Điều khiển TV livestream (user2): gắn SP vào chiến dịch + nhập số NCC báo (pending_qty).
    - uses shared: `Popup`, `Web2AuditLog`, `WEB2_CONFIG`, `API_CONFIG`, `Web2Auth`, `Web2Campaign`, `Web2Escape`, `Web2LiveTvDisplay`, `Web2Optimistic`, `Web2Perm`, `Web2ProductsApi`, `Web2Sidebar`, `Web2Skeleton`, `Web2SSE`, `Web2UserInfo`, `Web2VariantGroup`
    - funcs (50): $, add, addGroup, apply, applyRegionFilter, applyTvControlSse, boot, cartAvatarUrl, cartRowHtml, close, computeBoardOp, flatCodes, goTvPage, groupByKey, groupHtml, isAdmin, loadBoard, loadCampaigns, loadPicker, loadTvControl, miniCardHtml, normRegion, onBoardOp, onEsc, onLayoutInput, onSse, onSuccess, openCartDetail, openHistory, pickerItemHtml, refreshPickerAddedFlags, regionChipsHtml, regionOptions, renderBoard, renderPicker, renderTvCtl, rollback, run, runBoardOp, saveTvControl, scheduleBoard, schedulePicker, selectCampaign, setTvLayout, setTvRegion, snapshot, toast, tvPaginate, vrowHtml, wire

### web2/live-tv — WEB2.0 — TV livestream board (user1 xem). Realtime, read-only.

- **[live-tv.js](../../web2/live-tv/js/live-tv.js)** ·415 — WEB2.0 — TV livestream board (user1 xem). Realtime, read-only.
...
- **[overview.js](../../web2/overview/overview.js)** ·612 — WEB2.0 module — trang giới thiệu (landing) Web 2.0, phong cách Framer.
    - uses shared: `Web2Auth`, `Web2Escape`, `Web2UserProfile`
...
- **[reconcile-actions.js](../../web2/reconcile/js/reconcile-actions.js)** ·674 — WEB2.0 module.
    - exposes: `RC`
    - uses shared: `Popup`, `Web2EvidenceCamera`, `Web2UserInfo`
    - funcs (24): bindAuditUi, cancelPack, closeAuditModal, decrementPick, deliverOrder, fetchAudit, finalize, fmtTsFull, inputToTs, lockBody, normC, onScannerSubmit, openAuditModal, packOrder, pad2, renderAuditResults, resetPick, returnFailedOrder, selectPbh, shipOrder, syncAuditInputs, toggleManualPick, tsToInput, unlockBody
- **[reconcile-api.js](../../web2/reconcile/js/reconcile-api.js)** ·279 — WEB2.0 module.
    - exposes: `RC`
...
- **[reconcile-app.js](../../web2/reconcile/js/reconcile-app.js)** ·198 — WEB2.0 module.
    - uses shared: `Web2BarcodeScanner`, `Web2EvidenceCamera`, `Web2HistoryTimeline`, `Web2LabelOcr`
    - funcs (5): \_overlayOpen, bindUi, init, onResult, onScan
- **[reconcile-render.js](../../web2/reconcile/js/reconcile-render.js)** ·351 — WEB2.0 module.
    - exposes: `RC`
    - uses shared: `Web2AuditLog`, `Web2ImageLightbox`
    - funcs (6): b, open, renderActionButtons, renderDetail, renderLine, renderList
- **[reconcile-state.js](../../web2/reconcile/js/reconcile-state.js)** ·201 — WEB2.0 module.
    - exposes: `RC`
    - uses shared: `API_CONFIG`, `Web2Auth`, `Web2Escape`, `Web2Format`, `Web2HistoryTimeline`
...
- **[web2-campaign.js](../../web2/shared/web2-campaign.js)** ·232 — WEB2.0 shared.
...
- **[web2-evidence-camera.js](../../web2/shared/web2-evidence-camera.js)** ·224 — WEB2.0 module.
    - exposes: `Web2EvidenceCamera`
    - uses shared: `WEB2_CONFIG`, `API_CONFIG`
    - funcs (13): \_captureKbvision, \_captureWebcam, \_ensureVideoEl, \_findSidecar, \_log, blobToBase64, capture, init, onerror, onload, source, stop, warm
...
- **[web2-live-tv-display.js](../../web2/shared/web2-live-tv-display.js)** ·158 — WEB2.0 shared — quy tắc HIỂN THỊ màn TV livestream.
...
- **[web2-sidebar.js](../../web2/shared/web2-sidebar.js)** ⚠️983 — WEB2-clone sidebar for Web 2.0 pages.
    - exposes: `Web2Sidebar`
...
- **[web2-tryon.js](../../web2/shared/web2-tryon.js)** ·792 — WEB2.0 shared module.
    - exposes: `Web2Tryon`
...
| ------------------- | ------- | ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ensureStyles`      | 21      | → CSS shared / theme thay vì inject lặp               | native-orders-control-drawer.js, native-orders-kpi-health.js, web2-link-customer-modal.js, web2-manual-deposit.js, web2-pm-modal.js, web2-beauty-studio.js, popup.js, web2-barcode-scanner.js, web2-command-palette.js, web2-customer-chat-core.js, web2-drawer.js, web2-image-editor.js, web2-label-ocr.js, web2-logo-eraser.js, web2-order-tag-detail.js, web2-pack-counter.js, web2-product-counter.js, web2-qr-modal.js, web2-unread-panel.js, web2-wallet-balance.js, web2-zalo.js |
...
| `isAdmin`           | 12      |                                                       | native-orders-render.js, native-orders-state.js, ai-hub.js, web2-manual-deposit.js, goods-weight.js, live-control.js, overview.js, web2-kpi.js, web2-perm.js, web2-tryon.js, system-sse.js, web2-zalo-app.js                                                                                                                                                                                                                                                                            |
...
_…và 126 hàm trùng khác (xem web2-codemap.json)._
...
| [web2/live-control/js/live-control.js](../../web2/live-control/js/live-control.js)             | 1082 |
| [native-orders/js/native-orders-render.js](../../native-orders/js/native-orders-render.js)     | 1048 |
...
| [web2/shared/web2-sidebar.js](../../web2/shared/web2-sidebar.js)                               | 983  |
...
| [web2/cham-cong/js/cham-cong-app.js](../../web2/cham-cong/js/cham-cong-app.js)                 | 903  |
