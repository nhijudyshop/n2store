...
>
> **✅ BACKEND DEPLOY VERIFIED (2026-06-30 22:45):** `web2-api` (srv-d8n53oflk1mc739bi9gg) + `n2store-fallback` (srv-d4e5pd3gk3sc73bgv600) đều **LIVE trên `415e1eb3c`** (autoDeploy). Smoke 2 chiều: **no-token → 401** (services-overview · /customers/list · /native-orders/load — leak ĐÓNG) · **với token → 200** (services-overview · /customers/list+search · /native-orders/load — KHÔNG 401-regression). Gate + fail-closed chạy đúng.
>
