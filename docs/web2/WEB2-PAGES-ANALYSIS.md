...
- 🟦 **SO-firestore-1doc (C8) — DEFER có chủ đích** — migration kiến trúc multi-week (Firestore `web2_so_order/main` → Postgres normalized `so_orders/so_shipments/so_rows` + CRUD API auth + migration data + image storage + viết lại sync layer). = F1 trong [IMPROVEMENT-PLAN.md](IMPROVEMENT-PLAN.md). Half-migrate rủi ro data so-order đang dùng → làm thành 1 đợt riêng tập trung.
