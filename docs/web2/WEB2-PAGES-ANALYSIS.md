...
| #   | Bug                                                                                                                                                                                                                    | File:Line                                                       | Verify | Trạng thái |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- | ------ | ---------- |
| 3H6 | **Auto-snap CHẾT sau rework PUSH-only**: `_handleNewCommentAuto` subscribe `live:newComment` nhưng KHÔNG còn chỗ nào emit (luồng mới đi SSE→prependComments không emit). Chip "Auto: ON" xanh nhưng không chụp         | `live-livestream-snap.js:3732` + `live-comment-list.js:910-980` | ✓tay   | ⬜         |
| 3H7 | `prependComments` + `_appendOlderBatch` đọc `state.comments` thô không qua `_visibleComments` → comment người-bị-ẩn (mặc định 2 page shop) lọt realtime/scroll + offset lệch gây **dòng trùng khi cuộn**               | `live-comment-list.js:972, 181-188`                             | ✓✓     | ⬜         |
| 3H8 | Relay WS server KHÔNG auth: `/api/stop` kill realtime, `/api/start` inject client lạ, `/api/events*` leak PII khách (tên, fb_id, phone). Kèm: nút "+ Lưu vào Live" POST `/api/live-saved` **không tồn tại** → luôn 404 | `live-chat/server/server.js:761-874` + `live-api.js:287-311`    | ✓tay   | ⬜         |
| 3H9 | `offlineBatchAll` gán TẤT CẢ comment vào 1 video/broadcastStart (không group per-campaign như Force extract chip) → multi-campaign snapshot sai video + sai offset hàng loạt; auto-trigger silent khi campaign offline | `live-livestream-snap.js:708-770` + `live-init.js:687-703`      | ⚠ run1 | ⬜         |
...
| #    | Bug                                                                                                                                                                                               | File:Line                                               | Verify | Trạng thái |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | ------ | ---------- |
| 3H17 | web2-monitoring `/audit/:id/revert` (đảo tiền ví — processWithdraw), `/retry-queue/replay`, blacklist KHÔNG auth; `web2-match-audit.revert()` còn không transaction/FOR UPDATE → double-withdraw  | `v2/web2-monitoring.js:144-248` + `web2-match-audit.js` | ✓✓     | ⬜         |
| 3H18 | payment-signals `/approve` trên signal `dismissed`: guard chỉ chặn `confirmed` → **cộng ví + auto-reply khách** nhưng UPDATE `AND status='pending'` match 0 row → status kẹt dismissed            | `web2-payment-signals.js:420-424, 467-477`              | ✓tay   | ⬜         |
| 3H19 | **[REGRESSION fix H9]** kpi-assignments `loadUsers()` fetch không gửi `x-web2-token` (route giờ gate HARD) → 401 → dropdown NV rỗng → trang phân công KPI chết (hàm `authToken()` có sẵn dòng 52) | `web2/kpi/js/kpi-assignments.js:65-67`                  | ✓tay   | ⬜         |
...
| 3H21 | **Blocker bật ENFORCE**: `Web2Api._fetchJson` (mọi trang page-builder) + notification-bell + raw fetch native-orders/so-order KHÔNG gửi `x-web2-token`; web2-generic create/update/delete KHÔNG wire cả soft middleware; `userId/userName` history tin body client (spoof được) | `web2/shared/web2-api.js:16-29` + `web2-generic.js:329` | ✓✓     | ⬜         |

### 1C. HIGH/khác carry-over từ vòng 2 (verify vòng 3: VẪN MỞ)

- ⬜ **H11** — live-chat delta lọc `created_time >= since` → comment bị UPDATE (poller fill phone, has_order, sửa message) không render realtime. Cần `sinceUpdated`/cột `updated_at` + prepend merge theo id thay vì skip — `live-init.js` + `web2-live-comments.js:406`.
...
- ⬜ **LC-pollnow-auth** — `/poll-now` + `/campaigns*` + `/poller-pages` mutation không auth, fan-out 50 trang/post; cùng cụm: `livestream-snapshots` DELETE/offline-batch/extract-\* không auth (run1).
...
**Auth lỗ còn lại (cùng họ 3H14/3H17 — gắn soft là xong):** ⬜ `web2-cutout` 3 endpoint tách nền trả phí không auth (drain quota) · ⬜ `dashboard-kpi` GET không auth (lộ doanh thu + partner_name) · ⬜ `audit-log` GET /list không auth (lộ wallet adjustments + SĐT) · ⬜ notifications GET/list + mark-read không auth + mark-all **global** (xoá unread mọi user) · ⬜ pancake-refresh GET /status không gate (recon) · ⬜ poller-pages mutation không auth + không `_notify` · ⬜ backfill-supplier/backfill-short-codes không gate admin (lệch chuẩn S1).
