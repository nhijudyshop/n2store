...
> **✅ C8 PHASE 1 DONE (2026-06-13, `86c094955`+) — so-order Firestore → Postgres (web2Db).** Đã chuyển nguồn chuẩn từ Firestore `web2_so_order/main` → Postgres `web2_so_order` (1 doc JSONB + `version`), GIỮ shape `{tabs,activeTabId}` (chưa normalize ra rows — phase 2 sau). **Đã làm:** server `/api/web2-so-order` (GET /get + POST /save optimistic-version + POST /reset admin + SSE `web2:so-order` + `requireWeb2AuthSoft`) · client `so-order-storage.js` Sync chuyển sang Postgres (5 method public GIỮ NGUYÊN → so-order-app.js không đổi) + migration 1 lần từ Firestore + SSE realtime push. **Verify API prod:** /get→401 no-token, empty→v0, create→v1, STALE baseVersion→**409 conflict** (hết last-write-wins), correct→v2. **Cải thiện vs Firestore:** single source Postgres ✓, optimistic concurrency ✓, auth enforce ✓, realtime push ✓. **CÒN (phase 2, optional optimization):** normalize ra `so_shipments`/`so_rows` (per-shipment isolation + image object-storage) — chỉ cần khi data lớn/contention cao; hiện JSONB-doc đã giải quyết các vấn đề F1 cốt lõi.
>
> **Plan phase 2 (normalize — nếu cần sau):**
>
