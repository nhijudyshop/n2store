...
- **Client CHUNG (frontend) = `window.Web2ProductUnits`** ([web2/shared/web2-product-units.js](../../web2/shared/web2-product-units.js)) — NGUỒN DUY NHẤT gọi `/api/web2-product-units/*`: `resolve(target)` · `events(id)` · `byProduct(code)` · `ensure(codes)` · `reprint(ids,{userName})` · `attachForPrint(products,{qrBase,perItemQty,bumpReprint})`. Mọi trang (unit-scan, so-order-barcode, web2-products-render, web2-unit-reprint) dùng client này — KHÔNG fork fetch base+token nữa (gom 2026-06-29). `attachForPrint` gom luôn vòng gắn units giống nhau của so-order + Kho SP (khác mỗi `qrBase`/`perItemQty`).
