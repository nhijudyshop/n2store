...
- **Endpoints** `/api/web2-product-units`: `POST /ensure` (mint theo SL — MỚI) · `POST /mint` (legacy per-shipment) · `GET /resolve` · `GET /by-product/:code` · `POST /by-orders` · `GET /sort-manifest` (đơn chờ xếp kệ, cho Bàn chia hàng) · `GET /:id/events` · `POST /reprint` · `POST /assign` (manual, ít dùng) · `POST /:id/status` · `GET /clearance` + `POST /:id/clearance` (admin) · `POST /assign-auto`.
- **Client CHUNG (frontend) = `window.Web2ProductUnits`** ([web2/shared/web2-product-units.js](../../web2/shared/web2-product-units.js)) — NGUỒN DUY NHẤT gọi `/api/web2-product-units/*`: `resolve(target)` · `events(id)` · `byProduct(code)` · `sortManifest()` · `ensure(codes)` · `reprint(ids,{userName})` · `attachForPrint(products,{qrBase,perItemQty,bumpReprint})`. Mọi trang (unit-scan, sort-station, so-order-barcode, web2-products-render, web2-unit-reprint) dùng client này — KHÔNG fork fetch base+token nữa (gom 2026-06-29). `attachForPrint` gom luôn vòng gắn units giống nhau của so-order + Kho SP (khác mỗi `qrBase`/`perItemQty`).
- **Bàn chia hàng (put-wall sortation)** = [web2/sort-station/](../../web2/sort-station/index.html) (mobile, menu Bán Hàng). NV ngồi đống hàng quét QR từng món → **KỆ to + tên KH + beep/rung**, grid theo dõi **đủ/thiếu từng STT**, **manifest "mang ra kệ"** (gom theo STT, mang 1 lượt). Nguồn data `GET /sort-manifest` (units ASSIGNED gom theo đơn). Quét tra `unitToOrder` map tức thì (fallback `/resolve`); chống double-scan; SSE `web2:product-units` reload. Ý tưởng: put-wall / put-to-cart (cluster picking). Bước nâng cấp tương lai: DIY put-to-light (ESP32+WS2812 qua SSE) — xem ý tưởng D.
