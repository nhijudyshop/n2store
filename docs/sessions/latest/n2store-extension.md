# Latest Snapshot — `n2store-extension/`

> Snapshot tự động ghi đè sau mỗi commit chạm folder này. **Không edit thủ công.**
> Mục đích: khi session cũ chết (vd lỗi image limit), session mới chỉ cần đọc file này là có đủ context để tiếp tục.

**Latest session**: `RESUME:20260524-113959-fc9f6c4`
**Session file**: [`./20260524-113959-fc9f6c4.md`](../20260524-113959-fc9f6c4.md)
**Commit**: `fc9f6c4` — fix(snap-ext): manifest v1.0.6 — thêm <all_urls> host permission
**Last updated**: 2026-05-24 11:40:00 +07
**Summary**: fix(snap-ext): manifest v1.0.6 — thêm <all_urls> host permission

## Files changed in this commit (`n2store-extension/`)

- `n2store-extension/manifest.json`

## Last 5 commits touching `n2store-extension/`

- `fc9f6c436` fix(snap-ext): manifest v1.0.6 — thêm <all*urls> host permission *(2026-05-24)\_
- `d3f1d60c8` feat(snap-ext): N2Store Extension auto-capture tab — zero popup _(2026-05-24)_
- `411482c33` feat(domain): rewire codebase sang custom domain nhijudy.store _(2026-05-21)_
- `a5d448159` auto: session update _(2026-04-23)_
- `92e1b8249` fix(cors): full sweep — route all Render calls via Cloudflare Worker _(2026-04-22)_

---

**Để tiếp tục context trong session mới:**

1. Đọc file session ở trên để xem Files Modified + Next Steps đã điền (nếu Claude turn trước fill rồi).
2. Cần lùi xa hơn → `git show <sha>` theo list commit trên.
3. Hoặc paste token `RESUME:20260524-113959-fc9f6c4` cho Claude walk chain theo CLAUDE.md protocol.
