...
- **Actor irrelevant cho KPI** — chỉ beneficiary count. Bất kỳ NV nào add/remove SP cho đơn trong range của NV X → KPI vào X. Audit log giữ actor để trace ai thao tác.
- **PBH creation = reconciliation checkpoint** — trang `fastsaleorder-invoice` là ground truth gate. Manager review lúc tạo PBH catch các trường hợp thiếu/sai SP. Sau khi PBH confirmed, `actual_confirmed` events emit dựa trên products **final state** trong native_order — tin cậy state này thay vì replay từng forecast event.
