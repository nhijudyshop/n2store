...
| Gọi báo khách HH | Manual | Gọi báo khách hàng hết hàng, chờ phản hồi |
...
| Sub-tag | Mô tả |
|---|---|
| NCC hết hàng | Báo khách hết hàng hoặc đổi qua mẫu khác (chuyển từ Mục Xử Lý sang đây vì đơn coi như xã) |
...
- **ID cố định**: `T_MY` (không theo auto-increment T1, T2...)
- **Mục đích**: Đánh dấu đơn có món hàng từ inbox của My được thêm vào giỏ, đang chờ hàng về để đi chung với đơn hàng trên livestream của khách
- **Đặc điểm**:
  - Luôn có sẵn — không cần tạo thủ công, hệ thống tự inject khi load
  - **Hiển thị trực tiếp trong dropdown tag xử lý** (ở cuối cùng, dưới mục Đặc Điểm Đơn Hàng) — click để toggle on/off nhanh, không cần mở modal Tag T
  - Hiển thị trong panel chốt đơn (mục TAG T CHỜ HÀNG) như các tag T khác
  - **Ẩn khỏi modal "🏷️ QUẢN LÝ TAG T CHỜ HÀNG"** — không hiện trong danh sách, không đếm vào summary
  - Không thể xóa (nút × ẩn trong panel, chặn delete trong code)
- **Flow**: Seller click "📦 MY THÊM CHỜ VỀ" trong dropdown → tag được gắn → đơn ở Cat 1 auto chuyển sub-state "Chờ Hàng" → khi hàng về, seller click lại để gỡ → auto quay về "Okie Chờ Đi Đơn"
...
| "NCC hết hàng" (MỤC XỬ LÝ) | Chuyển sang MỤC KHÁCH XÃ SAU CHỐT | Reclassify |
| Không có flag báo khách HH | Flag "Gọi báo khách HH" (manual) | New |
| Không có tag T mặc định | "MY THÊM CHỜ VỀ" — tag T mặc định, toggle trực tiếp trong dropdown | New |
