...
> **⚠️ BƯỚC 0 — BẮT BUỘC TRƯỚC KHI CODE BẤT KỲ THỨ GÌ CỦA WEB 2.0 (2026-06-20)**: MỞ & ĐỌC thư mục [`web2/shared/`](web2/shared/) (≈90+ module dùng chung) — kèm [`docs/web2/WEB2-CODEMAP.md`](docs/web2/WEB2-CODEMAP.md) **§1 Shared Modules Registry** — để nắm **TOÀN BỘ module Web 2.0 đang có** TRƯỚC khi viết dòng nào. `web2/shared/` là nơi chứa MỌI capability dùng chung (chat KH, popup/confirm, SSE realtime, ví, NCC, QR, barcode/đếm SP, TTS giọng, optimistic UI, products/customers/suppliers cache, sidebar/page-builder, format/escape, …). **Quy trình**: (1) cần 1 capability → `ls web2/shared/` + tra CODEMAP §1 → có sẵn thì TÁI DÙNG, **KHÔNG viết lại / fork**; (2) chưa có mà ≥2 nơi cần → build shared 1 nguồn rồi gọi. Bỏ bước này = trùng lặp + drift + sửa-1-nơi-sót-nơi-khác. (Memory: [[feedback_web2_read_shared_first]].)
...
9. **Cập nhật file phân tích khi code phần QUAN TRỌNG Web 2.0 — BẮT BUỘC (2026-06-10, cập nhật 2026-06-27)**: khi thêm route/trang mới, đổi luồng data, hoặc fix bug nằm trong audit → cập nhật [`docs/web2/WEB2-PAGES-ANALYSIS.md`](docs/web2/WEB2-PAGES-ANALYSIS.md) (audit toàn diện 34 trang 2026-06-10 — fix bug nào thì đổi ⬜ → ✅ kèm commit sha ở dòng đó). Trước khi fix bug Web 2.0 → đọc file MD này TRƯỚC để biết bug đã được catalog chưa + pattern lỗi lặp (mục 2) + lộ trình 5 đợt (mục 5). ⚠ **`web2/overview/index.html` GIỜ LÀ TRANG GIỚI THIỆU** (Framer-style landing, login → đây; 2026-06-27) — KHÔNG còn chứa `#auditPages`/`#conventions`. Bản tài liệu cũ archive nguyên vẹn ở [`web2/overview/legacy-overview.html`](web2/overview/legacy-overview.html). Nguồn audit/tài liệu sống = `docs/web2/*.md` + dashboard `web2/system`.
...
- **🧰 [`docs/web2/MEDIA-KIT.md`](docs/web2/MEDIA-KIT.md) — KHO ĐA DỤNG media/AI (ĐỌC VÀO ĐÂY khi cần AI / Giọng nói / Hình ảnh / Video)**: gom theo 4 nhóm — capability → module global → file → API ngắn → "dùng khi"; đánh dấu ✅ shared vs ⚠️ feature-local (nên promote) vs 🛰️ backend. Cần TTS/clone giọng/nhạc/tách nhạc/render video/animation/beauty/QR/OCR/đếm SP/AI script → tra đây TRƯỚC, tái dùng KHÔNG viết lại. Lộ trình promote module media về `web2/shared/` ở cuối file.
- **🗺️ [`docs/web2/WEB2-CODEMAP.md`](docs/web2/WEB2-CODEMAP.md) — BẢN ĐỒ CODE Web 2.0 (auto-generated, ĐỌC TRƯỚC KHI CODE trang/feature/hàm Web 2.0 mới)**: liệt kê mọi trang/file + mục đích + hàm + global + shared đang dùng + **hàm trùng ≥3 file (ứng viên rút vào shared)** + file > 800 dòng. Quy trình: (1) cần 1 capability (chat/QR/popup/scan/ví/SSE/NCC…) → tra **§1 Shared Modules Registry** TRƯỚC, có sẵn thì TÁI DÙNG, KHÔNG viết lại; (2) cần biết trang có hàm gì/ở đâu → **§3 Pages**; (3) viết hàm mà tên đã có nơi khác → **§4 Hàm trùng** → cân nhắc shared. **Sinh lại sau khi đổi cấu trúc/ tách module / thêm trang: `node scripts/gen-web2-codemap.js`** (kèm `web2-codemap.json` cho máy đọc). Kế hoạch tách module toàn bộ: [`docs/web2/MODULARIZATION-PLAN.md`](docs/web2/MODULARIZATION-PLAN.md) + blueprint chi tiết [`docs/web2/modularization-blueprints.json`](docs/web2/modularization-blueprints.json).
- **🔌 [`docs/web2/WEB2-THIRD-PARTIES.md`](docs/web2/WEB2-THIRD-PARTIES.md) — TOÀN BỘ BÊN THỨ 3 Web 2.0 (auto-generated, ĐỌC khi cần biết app dùng API/thư viện/model/dự án GitHub nào)**: ~70 bên thứ 3 (audit 5 vòng) theo nhóm (AI/LLM, TTS, media-gen, stock, nhắn tin/TPOS, payment, CDN lib, model on-device, OSS GitHub, hạ tầng) + provider + cost + license + layer (web2/web1) + **`usedIn` file thật** + tên ENV key (KHÔNG giá trị) + GitHub repo. Trước khi tích hợp 1 dịch vụ/lib mới → **tra đây xem đã có chưa** (tái dùng key rotation/route sẵn). Nguồn curated: `web2/system/data/web2-third-parties.json`.
- **🧩 [`docs/web2/WEB2-PAGE-MODULES.md`](docs/web2/WEB2-PAGE-MODULES.md) — THỐNG KÊ TRANG ↔ MODULE shared (auto-generated)**: chiều 1 = mỗi trang import shared module nào; chiều 2 = mỗi shared module được trang nào dùng (đo độ phổ biến + phát hiện module mồ côi). Dùng để biết **sửa 1 shared module thì ảnh hưởng trang nào** (blast radius) trước khi đổi.
- **🖥️ Live dashboard [`web2/system/`](web2/system/index.html) (Cấu hình & Hệ thống)** — 5 tab: Dịch vụ&hạ tầng (DB live + chi phí), Realtime SSE, Các trang, **Module** (shared+trang+backend, lọc/tìm, hiện trang↔module), **Bên thứ 3** (lọc category/layer/cost). 3 doc trên + manifest `web2/system/data/*.json` **sinh lại bằng `node scripts/gen-web2-system-data.js`** (chạy SAU `gen-web2-codemap.js`). Sửa registry bên thứ 3 → sửa `web2/system/data/web2-third-parties.json` rồi chạy lại generator.
- **📓 [`docs/web2/KB-SYSTEM-SERVICES.md`](docs/web2/KB-SYSTEM-SERVICES.md) — KB Dịch vụ & Hạ tầng Web 2.0 (ĐỌC TRƯỚC KHI CODE `web2/system`, `services-overview.js`, hoặc đổi hạ tầng/SSE/bên-thứ-3)**: giải thích trang `web2/system?tab=services` (DB live + chi phí hardcode + modal chi tiết), topology backend (web2-api vs n2store-fallback, 2 DB pool, worker routing, cron), 2 hub SSE + topics, 70 bên thứ 3, shared modules chính, và §7 **gotchas dễ nhầm**. ⚠ Đây cũng là **KB-doc kiểu NotebookLM**: file markdown 1 nguồn → (a) Claude đọc trước khi code, (b) user upload lên NotebookLM để hỏi-đáp. Khi ghi "cái đáng chú ý" mới về 1 mảng → tạo thêm `docs/web2/KB-*.md` cùng pattern (NotebookLM KHÔNG có API để Claude đọc trực tiếp).
- **QUY ƯỚC WEB 2.0 canonical (ĐỌC TRƯỚC KHI CODE trang/feature Web 2.0 mới)**: DB/pool (`web2Db || chatDb`, KHÔNG ghi Web 1.0), naming (table `web2_`, route `/api/web2/`, service `web2-*.js`), realtime SSE `web2:<entity>`, server wiring, migration infra, checklist. Nguồn: section `#conventions` trong bản lưu trữ [`web2/overview/legacy-overview.html`](web2/overview/legacy-overview.html) + dashboard [`web2/system`](web2/system/index.html). ⚠ `web2/overview/index.html` (2026-06-27) GIỜ LÀ **trang giới thiệu Framer** (login → đây) — KHÔNG còn `#conventions`. Live: https://nhijudy.store/web2/overview/index.html
...
- Webhook/cron web2 truyền `web2Pool || chatDbPool`. Chi tiết quy ước: `legacy-overview.html` `#conventions` (bản lưu trữ) + `web2/system`.
...
- Lý do: overview (2026-06-27 đã thành **trang giới thiệu Framer**) vẫn load `web2-auth` (guard + base-URL) và lands thẳng Web 2.0 đã đăng nhập → tránh nhảy nhầm Web 1.0. Trang con tự load sidebar/SSE/theme riêng khi `nav` tới (overview mới KHÔNG mount sidebar). Vẫn dùng `--start web2/overview/index.html` làm landing.
