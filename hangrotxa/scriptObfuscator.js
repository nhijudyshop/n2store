...
async function migrateDataWithIDs() {
    try {
        ...
        if (hasChanges) {
            ...
            console.log(
                `Migration hoàn tất: Đã thêm ID cho ${migratedData.length} sản phẩm và sắp xếp theo thời gian`,
            );
...
function renderDataToTable(dataArray) {
    ...
    for (let i = 0; i < maxRender; i++) {
        ...
        button.setAttribute(
            "data-product-name",
            sanitizeInput(product.tenSanPham || ""),
        );
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                var imageRef = imagesRef.child(
                    file.name + generateUniqueFileName(),
                );
...
function addProduct(event) {
    ...
    const tenSanPham = sanitizeInput(
...
function testIDSystem() {
    ...
    testDates.forEach((dateStr) => {
        ...
        console.log(
            `"${dateStr}" -> ${parsed ? parsed.toLocaleString() : "null"}`,
        );
    });
...
function forceMigration() {
    ...
    const confirmMigration = confirm(
        "Bạn có chắc chắn muốn chạy migration?\nChỉ chạy khi cần thiết.",
    );
    if (confirmMigration) {
        //migrateDataWithIDs();
    }
