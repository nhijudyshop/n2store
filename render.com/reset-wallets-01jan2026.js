...
/**
 * Reset Wallets before 01/01/2026 11:00 AM
 */
...
const RESET_DATETIME = '2026-01-01 11:00:00';
const RESET_ID = 'RESET_01JAN2026_11AM';
...
async function resetWallets() {
    ...
    try {
        ...
        console.log('RESET VÍ TRƯỚC 11h 01/01/2026');
        ...
        for (const row of balanceBefore.rows) {
            ...
            if (real > 0 || virtual > 0) {
                await client.query(`
                    ...
                    VALUES ($1, $2, 'ADJUSTMENT', $3, $4, 0, $5, 0, 'MANUAL_ADJUSTMENT', 'system', $6, $7, '2026-01-01 10:59:59')
                `, [row.phone, row.wallet_id, -(real + virtual), real, virtual, RESET_ID,
                    'Điều chỉnh số dư trước 11h 01/01/2026 - Thật: ' + real.toLocaleString('vi-VN') + ', Ảo: ' + virtual.toLocaleString('vi-VN')]);
                adjustCount++;
            }
        }
        ...
        console.log('Đã cập nhật', recalcCount, 'ví từ giao dịch sau 01/01/2026');
