...
COMMENT ON COLUMN tickets.boom_reason IS 'Sub-reason for BOOM type tickets: BOOM_HANG, TRUNG_DON, DOI_DIA_CHI';

-- Create index for filtering by boom_reason (optional, for performance if needed)
CREATE INDEX IF NOT EXISTS idx_tickets_boom_reason ON tickets(boom_reason) WHERE boom_reason IS NOT NULL;
