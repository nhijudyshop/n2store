...
window.SoOrderCRUD = {
    ...
    // Import NCC names from Excel via tomato.tpos.vn API
    async importNCCFromExcel() {
        const config = window.SoOrderConfig;
        const state = window.SoOrderState;
        const utils = window.SoOrderUtils;
        ...
        if (!accessToken) {
            console.warn("No accessToken found in localStorage. Skipping NCC import from Excel.");
            return false;
        }

        try {
            console.log("Importing NCC names from Excel...");

            // Fetch Excel file from tomato.tpos.vn
            const response = await fetch("https://tomato.tpos.vn/Partner/ExportFile", {
                method: "POST",
                headers: {
                    "accept": "*/*",
                    ...
                    data: JSON.stringify({
                        Filter: {
                            ...
                            filters: [
                                ...
                                { field: "Active", operator: "eq", value: true, Clear: "Clear" }
                            ]
                        }
                    }),
                    supplier: true
            ...
            // Get the Excel file as ArrayBuffer
            ...
            // Extract NCC names from column D (index 3), skip header row
            const nccNames = [];
            for (let i = 1; i < data.length; i++) {
                const row = data[i];
                if (row && row[3]) { // Column D is index 3
                    const nccName = String(row[3]).trim();
                    const code = this.parseNCCCode(nccName);
                    if (code && nccName) {
                        nccNames.push({ code: code.toUpperCase(), name: nccName });
                    }
                }
            }

            if (nccNames.length === 0) {
                console.warn("No valid NCC names found in column D");
                return false;
            }

            console.log(`Found ${nccNames.length} NCC names from Excel`);

            // Save all NCC names to Firebase (batch write)
            const batch = config.db.batch();
            nccNames.forEach((ncc) => {
                const docRef = config.nccNamesCollectionRef.doc(ncc.code);
                batch.set(docRef, { name: ncc.name });
            });

            await batch.commit();

            // Update state
            state.nccNames = nccNames;
            state.nccNames.sort((a, b) => {
                const numA = parseInt(a.code.replace(/^A/i, "")) || 0;
                const numB = parseInt(b.code.replace(/^A/i, "")) || 0;
                return numA - numB;
            ...
            console.log("NCC names imported successfully from Excel");
            return true;
        } catch (error) {
            console.error("Error importing NCC from Excel:", error);
            return false;
        }
    },
};
