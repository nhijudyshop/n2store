...
const SERVICE = process.env.RENDER_SERVICE_NAME || process.env.SERVICE_NAME || 'n2store-live-chat';
...
async function sendAlert(tag, message, details = '') {
    ...
    try {
        const r = await fetch(`https://api.telegram.org/bot${TG_TOKEN}/sendMessage`, {
            ...
            body: JSON.stringify({ chat_id: TG_CHAT, text: body, parse_mode: 'Markdown', disable_web_page_preview: true }),
            signal: AbortSignal.timeout(5000)
        });
        if (!r.ok) console.warn('[ALERT] TG send failed:', r.status);
    } catch (e) {
