...
const LiveCommentList = {
    ...
    /**
     * Có nên animate dòng comment MỚI (fade+trượt dịu) không? Flow thường → có (dịu
     * mắt). BURST (comment dồn dập) → KHÔNG → hiện tức thì, tránh nháy loạn nhức mắt.
     * Burst = 1 batch > 5 dòng, HOẶC > 12 dòng animate trong 2s gần đây.
     * @param {number} batchN số dòng mới sắp chèn lần này
     * @returns {boolean}
     */
    ...
    prependComments(newComments) {
        ...
        // Hiệu ứng dòng MỚI: dịu mắt khi flow thường; TẮT khi burst (comment dồn dập)
        // → hiện tức thì, tránh nháy loạn. Gắn .is-new (CSS fade+trượt nhẹ) cho dòng
        // vừa chèn, tự gỡ sau animationend.
