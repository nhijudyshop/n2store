...
const PancakePageSelector = {
    ...
    renderPageSelector(pages) {
        ...
        const totalUnread = state.pagesWithUnread.reduce(
            (s, p) => s + (p.unread_conv_count || 0),
            0
        );

        let html =
            '<div class="pk-page-item all-pages ' +
            (!state.selectedPageId ? 'active' : '') +
            '" data-page-id="">' +
            '<div class="pk-all-pages-icon"><i data-lucide="layout-grid"></i></div>' +
            '<div class="pk-page-info"><div class="pk-page-name">Tất cả Pages</div><div class="pk-page-hint">' +
            pages.length +
            ' pages</div></div>' +
            (totalUnread > 0
                ? '<span class="pk-page-unread-badge">' + totalUnread + '</span>'
                : '') +
            '</div>';
        ...
        for (const page of pages) {
            ...
            const pu = state.pagesWithUnread.find(
                (p) =>
                    p.page_id === pageId ||
            ...
            const avatarUrl =
                page.avatar || (page.picture && page.picture.data ? page.picture.data.url : null);
            const avatarHtml = avatarUrl
                ? '<img src="' +
                  avatarUrl +
                  '" class="pk-page-avatar" alt="' +
                  escapeHtml(pageName) +
                  '">'
                : '<div class="pk-page-avatar-placeholder">' +
                  pageName.charAt(0).toUpperCase() +
                  '</div>';

            html +=
                '<div class="pk-page-item ' +
                (isActive ? 'active' : '') +
                '" data-page-id="' +
                pageId +
                '">' +
                avatarHtml +
                '<div class="pk-page-info"><div class="pk-page-name">' +
                escapeHtml(pageName) +
                '</div>' +
                '<div class="pk-page-hint">ID: ' +
                (page.fb_page_id || pageId) +
                '</div></div>' +
                (uc > 0 ? '<span class="pk-page-unread-badge">' + uc + '</span>' : '') +
                '</div>';
        }
    ...
    updateSelectedDisplay() {
        ...
        if (state.selectedPageId) {
            ...
            if (page) {
                ...
                if (avatarUrl) {
                    avatarEl.innerHTML =
                        '<img src="' +
                        avatarUrl +
                        '" class="pk-page-avatar" style="width:32px;height:32px;" alt="">';
                } else {
                    avatarEl.innerHTML = (page.name || 'P').charAt(0).toUpperCase();
                    avatarEl.className = 'pk-page-avatar-placeholder';
                }
                var pu = state.pagesWithUnread.find(function (p) {
                    return p.page_id === state.selectedPageId || p.page_id === page.fb_page_id;
                });
            ...
            avatarEl.className = 'pk-page-avatar-placeholder';
            var total = state.pagesWithUnread.reduce(function (s, p) {
                return s + (p.unread_conv_count || 0);
            }, 0);
