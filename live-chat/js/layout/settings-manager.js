...
/**
 * Settings Manager - Live and Pancake settings modals
...
const SettingsManager = (() => {
    ...
    function _initLiveSettings() {
        const btnOpen = document.getElementById('btnLiveSettings');
        const modal = document.getElementById('liveSettingsModal');
        const btnClose = modal?.querySelector('.close-modal');

        if (btnOpen) {
            btnOpen.addEventListener('click', () => {
                if (modal) modal.style.display = 'flex';
                _loadLiveSettingsValues();
            });
        }
        if (btnClose) {
