...
const ColumnManager = (() => {
    ...
    const COLUMNS = {
        live: { id: 'liveColumn', contentId: 'liveContent', name: 'Live', icon: 'shopping-cart' },
        pancake: {
            id: 'pancakeColumn',
    ...
    function applyOrder() {
        ...
        const liveCol = document.getElementById('liveColumn');
        const pancakeCol = document.getElementById('pancakeColumn');
        const handle = document.getElementById('resizeHandle');
        if (!container || !liveCol || !pancakeCol || !handle) return;

        container.innerHTML = '';
        currentOrder.forEach((key, i) => {
            container.appendChild(key === 'live' ? liveCol : pancakeCol);
    ...
    function _initSettingsPanel() {
        ...
        if (col1 && col2) {
            col1.addEventListener('change', () => {
                col2.value = col1.value === 'live' ? 'pancake' : 'live';
            });
            col2.addEventListener('change', () => {
                col1.value = col2.value === 'live' ? 'pancake' : 'live';
            });
        }
